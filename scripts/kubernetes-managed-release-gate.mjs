...
const requireFromApi = createRequire(
  path.join(rootDir, "wrapper", "api", "package.json"),
);
...
class ManagedReleaseGate {
  ...
  async exercisePersistence(baseUrl) {
    ...
    await requestJson(baseUrl, "/workflows/managed-release-workflow", {
      method: "POST",
      body: JSON.stringify({ input: "published" }),
    });
    await requestJson(baseUrl, "/workflows-latest/managed-release-workflow", {
      method: "POST",
    ...
    await waitFor(
      ...
      async () => {
        ...
        if (result.value?.environmentValue !== "managed-persistence")
          throw new Error(
            "Execution runtime did not receive the App Settings environment value",
          );
        return result;
      },
      30_000,
  ...
  async assertPersistedEnvironment(baseUrl, state) {
    ...
    await waitFor(
      ...
      async () => {
        ...
        if (result.value?.environmentValue !== "managed-persistence") {
          throw new Error(
            "Execution runtime did not receive the persisted App Settings environment value",
          );
        }
        return result;
      },
    ...
    await waitFor(
      ...
      async () => {
        ...
        if (runtimeResult.value?.environmentValue !== "managed-persistence") {
          throw new Error(
            "Replacement execution pod did not receive the persisted App Settings environment value",
          );
        }
