...
const launcherEnv = await import(new URL('../../../../deploy/studio-server/scripts/lib/docker-launcher-env.mjs', import.meta.url).href) as {
...
const devEnv = await import(new URL('../../../../deploy/studio-server/scripts/lib/dev-env.mjs', import.meta.url).href) as {
...
test('loadDevEnv honors explicit RIVET_ENV_FILE overrides and still derives filesystem host paths', () => {
  ...
  try {
    ...
    assert.equal(loaded.mergedEnv.RIVET_SOURCE_BUILD_CONTEXT_PATH, undefined);
    assert.equal(loaded.mergedEnv.RIVET_DEPENDENCY_BUILD_CONTEXT_PATH, undefined);
