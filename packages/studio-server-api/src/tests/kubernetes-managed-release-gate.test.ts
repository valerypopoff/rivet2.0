...
test("managed release gate reserves enough workers for deterministic execution node-drain coverage", async () => {
  ...
  const kindTopology = await fs.readFile(
    path.join(
      ...
      "studio-server",
      "scripts",
      "kind-managed-release-cluster.yaml",
    ),
    "utf8",
  );
...
test("managed release gate renders managed storage and digest-pinned image values", () => {
  ...
  assert.equal(
    imageReference(config.images.executor),
    `example.test/rivet/executor@${digest("d")}`,
  );
});
...
test("managed release gate allows preloaded local images without weakening the default pull policy", () => {
  ...
  assert.equal(
    renderManagedReleaseGateValues(localConfig).images.api.pullPolicy,
    "IfNotPresent",
  );
  assert.throws(
    () =>
...
function createProviderEnvironment(
  ...
  return {
    ...
    RIVET_K8S_PROVIDER_GATE_EXECUTOR_IMAGE_REPOSITORY:
      "example.test/rivet/executor",
...
test("managed release disruption gate covers WebSocket owner loss and managed dependency recovery", async () => {
  const [runner, fixture, overlay] = await Promise.all([
    fs.readFile(
      path.join(
        ...
        "studio-server",
        "scripts",
        "kubernetes-managed-release-gate.mjs",
      ),
      "utf8",
    ),
    fs.readFile(
  ...
  assert.match(
    runner,
    /rootDir,\s+"deploy",\s+"studio-server",\s+"scripts",\s+"fixtures",/,
  );
  ...
  assert.match(
    runner,
    /"interrupted WebSocket action replay",\s+async \(\) =>/,
  );
  ...
  assert.match(
    runner,
    /"workflow recovery after execution node drain",\s*\(\) =>[\s\S]*?this\.requestWorkflow/,
  );
  assert.match(
    runner,
  ...
  assert.equal(
    (runner.match(/this\.requestWorkflow\(/g) ?? []).length,
    7,
  );
  assert.match(
    runner,
    /result\?\.value\?\.type === "any" \? result\.value\.value : result/,
  );
...
test("provider staging gate is explicitly confirmed, HTTPS-only, and scoped to a staging namespace", async () => {
  const directory = await fs.mkdtemp(
    path.join(os.tmpdir(), "rivet-provider-gate-test-"),
  );
  ...
  try {
    ...
    assert.equal(
      providerConfig.interruptionManifests[0]?.restoreAction,
      "delete",
    );
    assert.equal(
      providerConfig.keyRotation?.nextSecretName,
      "rivet-settings-new",
    );
    assert.equal(
      providerConfig.registry.secretName,
    ...
    await fs.writeFile(
      configFile,
      JSON.stringify({ ...config, namespace: "rivet-production" }),
    );
    assert.throws(
      () =>
        buildManagedProviderGateConfig({
...
test("provider staging gate preserves secret inputs outside uploaded artifacts", async () => {
  const [runner, workflow] = await Promise.all([
    fs.readFile(
      path.join(
        ...
        "studio-server",
        "scripts",
        "kubernetes-managed-provider-gate.mjs",
      ),
      "utf8",
    ),
    fs.readFile(
      path.join(rootDir, ".github", "workflows", "studio-server-images.yml"),
      "utf8",
    ),
  ]);
  ...
  assert.match(
    runner,
    /reuseValues: true,\s+setValues: this\.getFinalAppSettingsKeyValues\(\)/,
  );
  assert.match(
    runner,
  ...
  assert.match(
    runner,
    /readiness recovery[\s\S]*?await this\.assertPublicSurface\(\);/,
  );
