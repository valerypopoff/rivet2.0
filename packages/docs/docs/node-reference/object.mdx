...
Interpolation supports the same JSONPath expressions as Extract Object Path and Destructure. For example, `{{customer.address.city}}` creates one `customer` input and selects `address.city` from its connected value.
...
When editing JSON Schema-style templates, you can Ctrl-click a field name inside a `required` array to jump to the matching definition under the sibling `properties` object. On macOS, use Cmd-click.
