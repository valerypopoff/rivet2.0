...
The Prompt Node is used to create a chat message, which is a string of text with an attached "type" indicating who sent the message (User, Assistant, System, or Developer) and optionally an attached "name".
...
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | ---------------- | --------------- |
| Type                 | The type of the chat message. Can be one of the following: `system`, `developer`, `user`, `assistant`, `function`. Developer messages express application instructions separately where the provider supports that role; other providers send them through their system-instruction mechanism. | `user`        | Yes              | `string`        |
...
The node body colors Markdown syntax and `{{interpolation}}` tokens like the Text Node. This is a preview of the prompt source, not rendered Markdown; the editor and generated message retain the full text even when the canvas preview is shortened.
...
The Prompt Node will error if the `Prompt Text` is not provided or if the `Type` is not one of the allowed types (`system`, `developer`, `user`, `assistant`, `function`).
