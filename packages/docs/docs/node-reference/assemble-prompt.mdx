...
| Is Last Message Cache Breakpoint | boolean                      | Whether to mark the final retained message as a cache breakpoint | N/A           | Available when the setting's input-port control is enabled       |
...
| Token Count | number         | Token count of the assembled prompt | Present only when **Compute Token Count** is enabled and calculated after optional empty-message filtering. |
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------- |
| Compute Token Count              | Adds a `Token Count` output containing the token count of the final assembled prompt.                                                                                                                  | Off     |
| Filter empty prompts             | Removes text-only messages whose content is empty or whitespace. Messages containing images, URLs, documents, assistant tool calls, or function responses are preserved even when their text is empty. | Off     |
| Is Last Message Cache Breakpoint | For providers that support prompt caching, marks the final message of a prompt containing at least two messages as a cache breakpoint. When filtering is enabled, this uses the retained messages.     | Off     |
...
A: The Assemble Prompt Node ignores missing and null inputs. Empty chat messages are included by default for compatibility with existing graphs. Enable **Filter empty prompts** to remove messages whose content is empty or whitespace-only. Rich-content messages and tool-protocol messages are retained because they remain meaningful even without text.
