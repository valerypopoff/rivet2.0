...
Enable Output request status when you want machine-readable response status/error outputs. With Retry on non-200 enabled, Response Status and Response Error become arrays so you can see what happened on each Rivet retry attempt. Rivet disables the Vercel AI SDK's hidden default retries, so the Retry on non-200 switch is the only retry policy for LLM Chat provider requests.
