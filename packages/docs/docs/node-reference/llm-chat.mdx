...
The API Key input port wins when it is enabled. In configured-key mode, the named environment variable is checked first so older graphs with different custom-provider credentials keep working; the Settings > LLM custom-provider key is the fallback.
