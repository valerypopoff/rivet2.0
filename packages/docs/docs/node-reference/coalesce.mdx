...
## Example 2: Getting a single value from a Match case node

1. Create a [Match case Node](./match-case.mdx) with a few different cases, say `1`, `2`, and `3`.
...
4. Connect an input to the Match case node so that only one of the cases will run.
5. Run the graph. Note that the output of the Coalesce node will be the value of the If node that ran.

The above is a common pattern for working with a Match case node - you can get a unique value for each case using an If node, which is not dependent on the value passed into the Match case node itself.
