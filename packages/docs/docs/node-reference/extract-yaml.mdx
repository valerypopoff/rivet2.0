...
The path uses the same safe structural-whitespace handling as interpolation and Extract Object Path. For example, `$ . yamlDocument . items [ 0 ]` and `$.yamlDocument.items[0]` select the same value; whitespace inside quoted values, regular expressions, filters, and object literals is preserved.

If the JSONPath expression is invalid, `Output` and `Matches` do not run and `No Match` returns the captured YAML text. If the path is valid but selects nothing, `Output` does not run, `Matches` returns an empty array, and `No Match` does not run. Invalid paths are handled as failed selections rather than thrown as graph errors.
