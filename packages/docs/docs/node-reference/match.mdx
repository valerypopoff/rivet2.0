...
If the `Test` input is missing, `null`, or `undefined`, the Match Node treats it as an unmatched value. It does not run any case outputs and executes the `Unmatched` output port. If no regular expression matches the test value, the `Unmatched` output port is also executed.
