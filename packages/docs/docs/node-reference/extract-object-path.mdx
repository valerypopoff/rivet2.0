...
JSONPath uses the same syntax and safe structural-whitespace handling as interpolation paths. For example, `$ . user [ "name" ]` is treated like `$.user["name"]`; whitespace inside quoted values, regular expressions, filters, and object literals is preserved.

If the path is invalid or the value is not found, the `match` output will not run and the `all_matches` output will run with an empty array. This fail-soft behavior lets the graph handle a missing or malformed selection through the normal output flow.
