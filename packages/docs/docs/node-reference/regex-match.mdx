...
4. Add three pairs of [If Nodes](./if.mdx) and [Text Nodes](./text.mdx). Connect the `Case 1` output of the Regex Match Node to the first If Node's `If` input, the `Case 2` output of the Regex Match Node to the second If Node's `If` input, and the `Unmatched` output of the Regex Match Node to the third If Node's `If` input. Connect the text nodes to the If Nodes' `Value` inputs.
