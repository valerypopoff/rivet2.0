...
Paths use the same safe structural-whitespace handling as interpolation and Extract Object Path. For example, `$ . user [ "name" ]` and `$.user["name"]` select the same value; whitespace inside quoted values, regular expressions, filters, and object literals is preserved.
