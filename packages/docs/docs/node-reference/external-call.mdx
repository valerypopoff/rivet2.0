...
when running Rivet in the Rivet application. The `setWebAppStatus` function is the
exception: Rivet provides it locally for graph progress, and web-app actions can
surface that progress in the preview. Use [Remote Debugging](../user-guide/remote-debugging.md)
