...
Web-app and host-persisted key/value state now uses the dedicated [Set Stored
Value](./set-stored-value.mdx) and [Get Stored Value](./get-stored-value.mdx)
nodes. The former `setWebAppStorage` and `getWebAppStorage` External Calls are not
installed automatically. This keeps storage behavior consistent in Browser, Node,
headless, nested-graph, detached-preview, and published web-app runs while allowing
API hosts to provide persistence callbacks.
...
ordinary editor runs: Rivet provides it locally for graph progress. Stored Value
nodes are also built in and do not require an external function. Use
