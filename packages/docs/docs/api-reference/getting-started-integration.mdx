...
- `knowledgeStores`: Optional named host implementations for Knowledge Source nodes. A host entry overrides project provider configuration with the same connection ID.
...
- Named custom-provider keys: If a custom-provider LLM Chat node sets `Alternative programmatic key name`, pass a top-level run option with that exact name.
- `openAiOrganization`: Optional OpenAI organization for OpenAI-backed paths.
- `openAiEndpoint`: Explicit legacy Chat endpoint override. Built-in LLM Chat and Generate using AI use provider-owned endpoints; use Custom provider settings for OpenAI-compatible hosts.
...
For multiple custom providers in one project, set each custom-provider LLM Chat node's `Alternative programmatic key name`, then pass matching top-level run options.

For authenticated or multi-tenant knowledge retrieval, provide a request-scoped `knowledgeStores` registry. The same option is available on `createProcessor`, `runRivetWebAppAction`, web-app handlers, and WebSocket sessions. See [Knowledge Sources](../user-guide/knowledge-sources.md).
