...
On a Rivet Server, access to a published web app is checked again when you start, cancel, or resume an action. If an administrator removes your access, unpublishes the app, or assigns its URL to a different app while it is open, you cannot start or control further actions. An action that was already accepted can finish safely, but the removed page no longer receives its progress or final result and will close; this also applies to a reconnect replay that was still loading when access changed. Other people who remain on the app's access list continue normally. Sign in again or reload the updated app instead. Republishing only the same app's code does not replace an already-open page's pinned version: it can finish or start actions against that version until you reload.

To show useful status while a graph works, put a **Report Progress** node on the workflow path. It passes its value through unchanged and can report a message, a percentage, or both. You can also use an **External Call** named `setWebAppStatus` and pass the message as its first argument. Progress appears only on the Button or Chat that started that run. In a Chat component, the separate progress panel uses the same text size as chat messages; wrapped status text keeps a slightly roomier bottom inset. It is temporary UI state, not a Graph Output and not project data.

For compact data that should persist in the user's browser, use [Set Stored Value](../node-reference/set-stored-value.mdx) and [Get Stored Value](../node-reference/get-stored-value.mdx). Set a key such as `preferences` and connect an object like `{ density: "compact", sidebarOpen: false }`; a later action can read the complete value using the same key. This works for settings, short local drafts, client-side caches, or other compact JSON data. Rivet isolates browser persistence by origin, app URL path, and UI graph ID. Values may contain strings, finite numbers, booleans, null, arrays, and plain objects. They are limited by the browser's local-storage quota, stay on that browser profile, are not synchronized across devices, may be cleared by the user, and should not contain secrets.

Hosted graphs receive the app's snapshot at action start and return successful changed-key patches. Browser and internal Node executor actions use the same behavior, and the desktop **Run detached** preview keeps storage ownership in the parent editor window even when Tauri isolates the child webview. In an ordinary editor graph run, stored values are shared only within that root run. API hosts can replace browser persistence with a `storedValueStore` implementation; callback-backed actions ignore the browser snapshot and return no browser storage changes.

The older `setWebAppStorage` and `getWebAppStorage` External Calls are no longer installed automatically. Replace them with the dedicated nodes.
