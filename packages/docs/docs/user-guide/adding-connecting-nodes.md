...
To manually bend a connection, hover the wire and click the ghost circle that appears. The circle becomes a bend handle; drag it anywhere on the canvas to route the wire through that point. Hold **Shift** while dragging to keep the handle aligned horizontally or vertically with where the drag started, just like dragging a node. Double-click the bend handle to remove it and return the connection to its automatic route. Bend handles are saved in the project file as visual layout only: they do not change which ports are connected or how the graph runs.

To move several bend handles and nodes together, hold **Shift** and drag a selection rectangle around them, then drag any selected node or handle. Bend handles are selected only by this rectangle, not by clicking them. Dragging an unselected handle moves only that handle. Click an empty part of the canvas to clear the selection. Undo/Redo restores the entire group move in one step.
