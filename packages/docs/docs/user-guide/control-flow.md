...
Many nodes can return a `control-flow-excluded` value, such as a [Match case Node](../node-reference/match-case.mdx) (for branches that do not match), and an [Extract Object Path Node](../node-reference/extract-object-path.mdx) (for when the input path is invalid for a given object).
...
- [Coalesce](../node-reference/coalesce-new.mdx) - `control-flow-excluded` values are skipped, and the next input that actually ran is passed through even when its value is falsy, empty, `null`, or `undefined`. Enable `Ignore 'null'` or `Ignore 'undefined'` on the Coalesce node when those payloads should also be skipped.
