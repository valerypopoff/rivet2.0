...
The sparkles button in the bottom-left corner of the canvas opens a similar AI modal for the current graph itself. Use it to describe nodes or graph edits you want Rivet to create. This graph creator uses the same model and credentials from **Settings > LLM**, but instead of drafting text for one editor field it runs Rivet's graph-building helper and applies graph changes to the current canvas.
