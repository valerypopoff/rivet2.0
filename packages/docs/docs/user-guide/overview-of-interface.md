...
Text and code editors have their own font-size control, separate from the main app UI font size. Use the **+** and **-** buttons in the editor footer to change the editor font size, or press **Ctrl + +** / **Cmd + +** and **Ctrl + -** / **Cmd + -** while focus is inside the editor. **Ctrl + 0** / **Cmd + 0** resets the size. You can also hold **Ctrl** or **Cmd** and use the mouse wheel or trackpad scroll while the pointer is over the editor to scale the editor font. Read-only code and JSON blocks in the full output modal remember their own font size separately, using the same keyboard and mouse-wheel shortcuts while focus or the pointer is inside that output block. In a full output modal, **Page Down** and **Page Up** smoothly move between whole output sections or response items, while **Home** and **End** smoothly go to the beginning and end. These shortcuts do not replace normal editing keys when an input or code editor has focus.
...
The sparkles button in the bottom-left corner of the canvas opens a similar AI modal for the current graph itself. Use it to describe nodes or graph edits you want Rivet to create. The Graph Builder uses the same model and credentials from **Settings > LLM**.

The new transactional implementation is available during rollout through **Settings > Graphs > Graph Builder implementation**. In **Transactional** mode, Graph Builder works on a private draft instead of changing the open graph while the model is still working. It may ask a clarification question when the request is ambiguous. When a valid draft is ready, review its semantic summary and diagnostics, then choose **Apply** to publish the whole change as one undoable editor action, **Discard** to leave the graph untouched, or **Start over** to make a different request. Canceling generation or closing and confirming Discard also leaves the authoritative graph unchanged. **Legacy rollback** remains the default until the transactional implementation passes its evaluation and dogfood rollout gates. It preserves the older model-and-tool orchestration, but now uses the same private-draft review and atomic **Apply** boundary; the implementation selector does not weaken publication safety.

Graph content selected for an authorized read and the request itself are sent to the configured model provider. Rivet omits configured credentials, classified secret fields, and unknown plugin settings from the Graph Builder's default model-visible projection.
