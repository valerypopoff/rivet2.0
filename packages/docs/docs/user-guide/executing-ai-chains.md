...
- You may want to take different actions depending on what your extracted value is. For this, use the [Match case Node](../node-reference/match-case) for exact values or switch it to regular-expression mode for patterns. Or, you can use an [If/Else Node](../node-reference/if-else) to get fallback values.
