...
- the selected executor mode is remembered per open project tab in editor-owned metadata, not in `.rivet-project` YAML; Browser/Node selector changes and Remote Debugger connect/disconnect write the active project's stored executor mode immediately, while tab activation restores local modes through `selectedExecutorState` and remote modes through the project-scoped executor-session runtime. This lets multiple tabs keep independent executor targets, including Remote Debugger tabs that use the same websocket URL/port
...
clearing pre-existing unsaved graph edits. Use `setProjectTabUiState(...)`, or
the `tabUi` option on `openProjectSnapshot(...)` / `replaceCurrent(...)`, for
transient wrapper-owned tab presentation state such as preview tabs; this state
is in-memory only and is cleared when the project tab closes. The handle also exposes
