...
Recordings preserve incoming `durationMs` and `splitRunDurationMs` when present. [`RecordingPlayer`](../packages/core/src/model/RecordingPlayer.ts) can also derive replay-only legacy aggregate durations from existing recorded `nodeStart.ts` and terminal event `ts` values; that fallback is not used for live remote-debugger traffic where receive timing would be misleading. During playback it also exposes each original `RecordedEvent.ts` as transient `replayRecordedAt` provenance. That metadata lets the editor show the historical root duration without slowing playback, and `ExecutionRecorder` never serializes it into a replay-recorded recording.
...
each string-list row. A stored-stable binding may also declare
...
The built-in `image` node defaults to its existing binary/file source when `sourceType` is absent in a saved project. Base64 mode uses a separate raw-text code setting and optional `base64` string input; the binary `data` port keeps its original ID and type when that mode is selected, so older graph connections remain valid. Only the selected source contributes bytes to the image output, while the Media Type setting/input applies in both modes. The editor hides the inactive source field and shows a selected-file status without rendering a thumbnail in settings. The canvas body summarizes the configured source and media type without duplicating the image; the inline execution output renders the resulting image within the node width and a bounded preview height, leaving fullscreen output unchanged.

`toBase64` is a separate built-in with a required `any` input, `preserve-array` behavior, and a stable `base64` string output. A connected array is encoded as one JSON value, even in Split Run mode; the usual Split Run output aggregation still applies. It encodes `binary`, `image`, `audio`, and `document` bytes directly, as well as `ArrayBuffer`, byte views, and `Blob` values carried in `any`; strings as UTF-8; and other JSON-serializable values as compact JSON in UTF-8. Its JSON branch accepts plain records even from another JavaScript realm, but rejects non-finite numbers and opaque object instances, including nested byte containers, instead of silently converting them to `null` or `{}`; ordinary JSON omission rules still apply to optional `undefined` fields. The shared byte encoder must support Node, browser main threads, and browser workers: workers have `btoa` but neither `window` nor `FileReader`. Its worker fallback encodes three-byte-aligned chunks to avoid constructing a second full-size binary string or introducing base64 padding between chunks.

`Code` (internal type `codeNew`), `Expression`, `JS Filter`, and `JS Map` are core built-ins that evaluate JavaScript through `context.codeRunner` and share value-backed interpolation from [`jsValueInterpolation.ts`](../packages/core/src/model/nodes/jsValueInterpolation.ts). `{{var}}` and `{{var.path[0]}}` create one `any` input port for the base `var`, then evaluate the JSONPath suffix against that connected value through a collision-safe, narrow interpolation helper injected by each runner. This keeps browser, Node, cached Node, managed hosted, and desktop-worker execution on the same semantics without giving callback nodes the full `Rivet` namespace or pulling them out of their worker path. Input objects/arrays are cloned before evaluation so expression-side, code-body-side, or callback-side mutation cannot mutate upstream graph data. Function-valued inputs are wrapped so property mutation stays local, but invoking a function can still perform whatever side effects that function implements. The shared helper owns interpolation input-definition creation, safe internal input identifier selection, cloned-input initializer generation, parsed-source preview interpolation, and generated-error sanitization. Generated interpolation helper identifiers are chosen so they do not collide with identifiers already present in the authored source, which keeps user locals or callback parameters from shadowing interpolation values. Missing interpolation inputs become `undefined`. `Code`, `Expression`, `JS Filter`, and `JS Map` still own their wrapper shapes and output contracts explicitly; do not replace them with a generic JS-node wrapper unless behavior is intentionally being redesigned. `Code` differs from `Expression` by wrapping the authored source in an async function body so users can declare locals, use `await`, and `return` one value. Code, Code (legacy), and Expression always request every `CodeRunner` API. Node runners provide `console`, `fetch`, `Rivet`, `require`, and `process`; browser runners provide `console`, `fetch`, and `Rivet`, while authored use of `require` or `process` fails only when it is evaluated there. This executor boundary is not a sandbox for untrusted JavaScript. Graph Builder can author their source and configured ports without hidden runtime-permission settings. The older internal `code` node is user-facing `Code (legacy)` and keeps the manually configured inputs/outputs contract. User documentation intentionally keeps both pages: `node-reference/code-new` is the current Code node, while `node-reference/code` is Code (legacy), so existing external docs links stay valid while the sidebar labels match the UI.

`CodeRunnerOptions.interpolationHelperIdentifier` is a narrow runner-extension contract for new path and special-root JavaScript interpolation. Built-in runners inject `resolveCodeInterpolationExpression` under that requested collision-safe identifier; a third-party runner that supports `{{foo.path}}`, `{{@graphInputs.*}}`, or `{{@context.*}}` must do the same. Ordinary bare `{{foo}}` references remain generated from cloned `inputs` directly and do not request this hook, preserving older custom-runner behavior without duplicating the JSONPath resolver.

`@globals.*` is the third portless interpolation root beside `@graphInputs.*` and `@context.*`. `getInterpolationGlobalValues(...)` captures only the IDs referenced by a template from the same root-run map as Get Global and Set Global, at node execution time. It never waits, creates a graph dependency, retains a pruned writer, or infers ordering; a writer must remain explicitly ordered when a read depends on it. JavaScript wrappers clone that selected snapshot before use. When a Code-family template includes `@globals`, `CodeRunnerOptions.globalValuesIdentifier` requests a second collision-safe runner argument beside `interpolationHelperIdentifier`; every built-in browser, Node, cached Node, managed hosted, and desktop-worker runner injects the selected snapshot under that name. A third-party runner that supports special-root interpolation must honor both identifiers for `@globals`; otherwise the generated wrapper fails with a targeted extension-contract error instead of silently treating the globals as missing. Add every generated globals identifier to the Code-family diagnostic sanitizer, alongside the inputs, clone-cache, graph-inputs, context, and resolver identifiers, so wrapper internals never leak into user-visible errors.

`JS Filter` and `JS Map` share editor, input, body-preview, wrapper, interpolation, and output-validation scaffolding in [`jsListCallbackHelpers.ts`](../packages/core/src/model/nodes/jsListCallbackHelpers.ts). Their settings editor uses the generic code-editor contract with a pre-editor callback-signature helper, a post-editor interpolation note, and `interpolationSyntax: 'js-value'`, keeping the body-only UX in core metadata instead of custom app UI. Their node-specific editor tests should assert that same metadata so changes to the shared helper cannot silently drift away from the node contracts. Their callback bodies are still wrapped explicitly by each node so the filter/map runtime differences remain inspectable. The callback-local names `item`, `index`, and `array` are reserved and do not create input ports; `{{item}}`, `{{item.customer.name}}`, `{{index}}`, and `{{array[0]}}` resolve through the same JSONPath interpolation resolver against the existing callback parameters. App-side parsed-callback previews use their own presentation wrapper instead of expanding the core helper API; the core helper stays focused on runtime and node-body contracts.
...
Interpolation expressions are parsed and resolved centrally in [`packages/core/src/utils/interpolation.ts`](../packages/core/src/utils/interpolation.ts), with JSONPath normalization and evaluation delegated to [`jsonPath.ts`](../packages/core/src/utils/jsonPath.ts), the same `jsonpath-plus` boundary used by Extract Object Path, Destructure, and Extract YAML. `normalizeJsonPathExpression(...)` is the single owner of Rivet's compatibility whitespace around structural separators: forms such as `{{foo . items [ 0 ]}}` and `$ . items [ 0 ]` normalize identically, while quoted values, regular expressions, filter expressions, and object literals retain their authored contents. Runtime JSONPath consumers must call `evaluateJsonPath(...)` rather than invoking `jsonpath-plus` or implementing local normalization. The dependency-light [`interpolationSyntax.ts`](../packages/core/src/utils/interpolationSyntax.ts) subpath owns token-boundary scanning and is also consumed by Monaco, so editor markers cannot drift from Core's escaping, JSONPath filter, or malformed-token recovery rules without pulling in JSONPath evaluation. An ordinary expression has a base identifier plus an optional JSONPath suffix: `{{foo.bar.some[0]}}` and `{{foo[?(@.active)].name}}` both create only a `foo` port. `@context.*` and `@graphInputs.*` remain portless roots. The resolver uses JSONPath `wrap: false` behavior (no match is `undefined`, one match is a scalar unless the selected value is itself an array, and multiple matches are an array), while each consumer retains its existing string or JSON-template presentation behavior. Do not parse paths with lodash, regex, or node-local splitting; use the shared parsed-expression/raw-resolution APIs so filters, recursive descent, bracket notation, processing chains, and escaped tokens cannot drift. Two bounded syntax-only caches keep immutable parsed data: one is keyed by exact template text and stores token spans/tokens/references; the smaller one is keyed by trimmed expression text and stores parsed base/path records. Neither can retain graph values, DataValues, or processor state. Extraction arrays and individual parsed-expression/reference helpers return safe copies; `parseInterpolationTemplate(...)` intentionally exposes the immutable cached syntax for allocation-free internal consumers. This is a deliberate compatibility boundary: a historical literal port named `foo.bar` is now interpreted as `foo` plus a path and has no automatic migration.

Input definitions that are generated from user-authored `{{var}}` interpolation and exposed as connectable input ports must be created through [`packages/core/src/model/interpolationInputDefinition.ts`](../packages/core/src/model/interpolationInputDefinition.ts). This is a core maintainability rule for all current and future nodes, not just a convenience helper. The helper preserves the existing port id/title contract while adding `NodeInputDefinition.data` metadata that says the port came from interpolation and records the base interpolation name. The app relies on that metadata to preserve connections when a user clearly renames an interpolation base; changing only a suffix such as `{{foo.name}}` to `{{foo.email}}` must leave the `foo` connection untouched. The metadata is runtime/editor-only and is not persisted in graph files. Built-in interpolation producers currently marked this way include Text, Prompt, Object, Tool schema interpolation, Code, Expression, JS Filter, JS Map, Extract Object Path, MCP argument templates, and the built-in OpenAI Thread Message plugin node. Nodes such as `To Tree` that use `{{...}}` only against per-item object properties do not expose connectable interpolation ports and therefore should not use this marker.

Interpolation safety coverage should stay broad whenever this contract changes: core tests cover parser edge cases, JSONPath result shape, processing chains, namespace roots, and built-in input-definition marking; negative tests cover runtime-only consumers such as `To Tree`; runner tests cover browser, Node, cached Node, managed, and desktop-worker helper injection; app graph-editing tests cover base-port preservation and rename recovery for marked built-ins and plugin nodes; and execution-data tests cover parsed-source previews using the captured run inputs rather than current editor state.

Dynamic interpolation port discovery reuses the bounded template cache in the shared parser. Its immutable parsed template records contain spans, token syntax, and base/path references rather than just extracted names; the separate expression cache is smaller and syntax-only. Neither cache holds node definitions, graph state, runtime values, or plugin results. When the template cache is full, new templates parse normally first; a repeated immediate miss can clear stale entries and admit the newly hot template rather than evicting on every miss. Callers receive fresh extraction arrays and copies of individual parsed-expression records, while cached template records must be treated as read-only. Keep plugin node definitions and connection-sensitive built-ins out of this cache path unless a future API adds an explicit cache-safety contract.
...
- `chatV2ProviderProfile.ts` is the shared credential/provider boundary for LLM Chat and app AI-assist adapters. It applies one precedence table and returns the live credential separately from a secret-free provider profile; neither request-plan summaries nor catalog keys may contain the credential value. Provider Advanced Headers travel through that same boundary into the OpenAI, Anthropic, Google, Custom Completions, or Custom Responses provider factory, and every physical request therefore receives them across retries, fallback candidates, and tool-continuation rounds.
...
- `chatV2RequestPlan.ts` is the pure transport-policy boundary. It selects stream versus generate, normalizes the explicit Rivet retry policy, fixes Vercel AI SDK retries at zero, and carries tools/response format/provider options/generation/output policy into `chatV2Pipeline.ts`. It deliberately excludes abort signals, SDK executors, and all request/partial/complete-response observers; the pipeline installs those invocation-local callbacks for each physical attempt instead of allowing a cached policy object to retain them.
...
- LLM Chat defaults missing `configurationMode` to the legacy Inline behavior. In From profile mode, the node accepts one typed `llm-config` input or an ordered `llm-config[]` fallback chain, merges only declaratively profile-owned fields into a temporary effective node-data object, and then enters the same provider-neutral planning, request, output, continuation, and cache paths as Inline mode. `llmProfileFallback.ts` owns retry-before-advance candidate selection for each model round, forward-only stickiness after a candidate succeeds, `LLM Attempts` facts, and exhausted-chain behavior. The profile-chain input explicitly preserves its array under split runs instead of creating one run per candidate. Stale inline provider ports are hidden and ignored. The editor-cache identity fingerprints every ordered profile credential without retaining raw keys; exhausted chain results are not cached.
- `chatV2Errors.ts` owns provider/Vercel SDK error normalization, including API-call and browser/runtime fetch-failure classification for retry decisions where no HTTP response is observable. It extracts HTTP status codes from common raw/normalized error shapes for retry. When an API response is observable, it preserves the provider's original message, preferring the response body over generic SDK data and following nested causes; Rivet's recommendation is supplemental and must not suppress an identical SDK/provider message. Developer-facing LLM diagnostics deliberately retain complete provider messages and endpoints.
...
- `chatV2Outputs.ts` owns provider-neutral output assembly: `Response` typing for structured formats, assistant/function-call outputs, usage/cost normalization, reusable control-flow exclusion for absent optional outputs, reasoning exclusion, and request/response body outputs. `llmInvocationResultProjector.ts` adds the opt-in universal `LLM Attempts` record after execution facts have been collected.
- `chatV2Pipeline.ts` executes `ChatV2RequestPlan` and stays focused on provider-neutral orchestration, retry outcomes, provider-error decisions, and output assembly; its discriminated execution result lets the profile fallback coordinator advance after an unsuccessful candidate while terminal failures remain node errors. `toolContinuation.ts` owns continuation. `aiSdkBridge.ts` adapts the planned request to Vercel AI SDK call signatures and retains a defensive zero-retry default for direct bridge callers. Rivet's request plan/retry loop is the single source of retry behavior and `LLM Attempts` records every physical/configuration/validation decision. For structured response formats with an SDK output descriptor, `aiSdkBridge.ts` resolves the AI SDK's parsed `output` promise on a best-effort basis and `chatV2Outputs.ts` uses that parsed value for the `Response` output while keeping the assistant message text unchanged for chat history. Parsed-output failures fall back to the response text as a string instead of failing the node. Structured-output calls also ask `consumeAiSdkStream(...)` to collapse exact duplicate text blocks and normalize repeated parseable JSON text before partial-output updates or fallback parsing, because some AI SDK/provider combinations expose the same final JSON object more than once.
- JSON schema mode always requires the final `Response` Data Value to be an object. `chatV2Pipeline.ts` applies that check only after `chatV2Outputs.ts` has assembled the exact parsed-or-string value for the output port. A failure is not a transport retry: `llmProfileFallback.ts` records a distinct response-validation attempt and immediately advances to the next candidate, while Inline or scalar-profile execution throws the detailed validation error.
...
- `providerOptions.ts` owns both Custom-provider protocols. Missing or `completions` keeps the backward-compatible `@ai-sdk/openai-compatible` Chat Completions path; opt-in `responses` uses the `@ai-sdk/openai` Responses model with provider name `custom`. Both paths reuse the normalized Custom base URL, credential/header resolution, diagnostic fetch wrapper, retries, fallback chain, streaming, and continuation pipeline. Custom Completions JSON object mode is represented by generated `providerOptions.custom.response_format` instead of an SDK output descriptor, while Custom Responses uses the OpenAI adapter's structured-output contract. Resolved **Extra provider options** never enter an adapter namespace: the fetch wrapper applies them as an exact top-level overlay after SDK request generation and Rivet role restoration, then captures and sends that same final body. The Completions factory does not request `stream_options.include_usage`; compatible providers can still return usage naturally, but Rivet does not force optional OpenAI stream metadata onto every custom endpoint. Provider factories also own the parsed request/response diagnostic hooks used by LLM Chat: the request hook records the body handed to the actual provider HTTP call for `LLM request body` and deliberately omits request headers / Rivet-managed API key headers. The opt-in response hook clones each received `Response`, so `LLM response body` can parse the exact raw provider body without consuming the AI SDK stream. Its collector waits only at the completed provider-call boundary, preserves physical response order across retries and tool rounds, and silently omits an unclonable response rather than changing request behavior.
...
Auto-delegation prefers a graph whose metadata name exactly equals the tool name. If no exact match exists, it preserves the legacy behavior by selecting the first graph whose name contains the tool name. Keep editor reachability analysis and Tool-node missing-handler warnings aligned with this exact-first, contains-fallback order so they identify the same graph that runtime dispatch will execute. The Tool warning is concise: `Auto Delegate needs a graph named "<tool>" for Tool "<tool>".`

Each live handler-subgraph or external-function cost is carried only on `ToolCallDelegationResult`, copied to that invocation's hidden Delegate `cost` output, and accumulated exactly once into the owning graph. `DelegatedToolCallRecord` intentionally excludes cost: cached or replayed records expose their stored tool result without charging an earlier execution again. The record retains the normalized argument object that was passed to the handler, so connected continuation, ordinary downstream delegation, and replay expose the same tool-call observability data.

The Delegate's persisted `message` output id is retained and displayed as `Tool Result Message`. Its persisted `assistant-message` output is displayed as `Message` and carries nonblank assistant text emitted in the same model response as the tool calls. The output definition and runtime-object order is `tool-name`, `tool-arguments`, `assistant-message`, `output`, `execution-time`, then `message`, so inline and fullscreen generic output rendering match the visible port order. `Tool Name` identifies the selected tool; `Tool Arguments` emits the normalized object actually passed to its handler; `Tool Execution Time (sec)` emits the seconds spent running that handler graph or external function. The pre-tool Message output is intrinsically per-call and pre-tool: every invocation emits the shared assistant text under its own process id, then starts that Message branch and its tool handler in parallel. A `Start Async Branch` boundary returns immediately and registers its remaining work with the root run when the branch must not hold the foreground path open. After each invocation's `nodeFinish`, its scalar Tool Name, Tool Arguments, Output, Tool Execution Time (sec), and Tool Result Message branches execute independently. Whitespace-only assistant text does not activate the pre-tool branch.
...
`ProjectMetadata.globalVariables` adds optional authored global variables to the same root-run global map used by `Get Global` and `Set Global`. [`GlobalVariables.ts`](../packages/core/src/model/GlobalVariables.ts) is the only portable-codec and validation boundary: every non-function type must match its declared scalar or array shape, IDs must be nonblank, and literals may contain only portable values. It preserves `undefined`, non-finite numbers, and nested `Uint8Array` values through private markers; raw non-finite numbers, sparse/circular values, callables, and class instances are rejected before serialization or execution. Both V4 project-file boundaries and direct root-run initialization use this validation, so no executor accepts a definition that another path would reject. IDs such as `__proto__` remain valid, but record writes must use safe own-property creation. Before a root run processes nodes, `GraphProcessor` loads references, verifies each loader result matches the requested project ID, resolves each project once in declared depth-first order, and applies the root definitions last. Legacy back-edges are cycle-tolerant: the in-memory root is already visited, so a child-to-root edge never asks the loader to reload the active project. All definitions are decoded before globals change; processor reuse removes only prior configured keys and preserves unrelated run-local globals. Subprocessors inherit that populated map and never assign them again. After root/graph start but before preloaded or ordinary node work, the processor emits normal `globalSet` events with process id `initial-project-global-variable`; recordings therefore replay the actual initialization without fabricated node lifecycle events. A normal or frozen Set Global remains the later authoritative writer and can overwrite a project global variable. Playback itself only re-emits recorded events and never applies the project configuration anew.

The app-side Get Global variable selector scans configured project-global IDs, loaded referenced-project global IDs, and fixed `Set Global` node IDs. It overlays the live open graph over the saved project graph list so unsaved Set Global edits are searchable immediately, and writes a selected ID into the normal `id` node data field; dynamic IDs supplied through the `Set Global` ID input port are runtime values and cannot be discovered safely by the editor. Entering or selecting an exact known ID also copies an editor-time type suggestion into Get Global's existing `dataType` field: project settings first, enabled fixed-ID setters in the live graph then other graphs in stable graph/node-ID order, then referenced project definitions in stable project-ID order. Conflicting declarations are reported in a hint below the type control. The hint is saved with the Get Global node, remains visible across editor reopening, and is removed when the user manually changes the type or variable ID. The selected type is a captured port/default type, not a runtime inference or a guarantee about later Set Global writes; later declaration edits do not silently rewrite Get Global, and a manual type selection remains saved. Input-driven IDs have no suggestion. The optional editor-only `typeSuggestion` data preserves the hint across reopening and project saves; older Get Global nodes require no migration, and port IDs are unchanged. A configured project global also satisfies the passive missing-Get-Global warning. Variable ID input ports stay string-typed regardless of the selected global value data type. The Get Global value output displays the fixed variable ID as its title, falling back to `Value` for an input-supplied or blank ID; its port ID and process output key remain `value`. Set Global displays its fixed ID on `saved-value` and `Prev value of: <ID>` on `previous-value`, falling back to `Value` and `Previous Value` for input-supplied or blank IDs. In input-driven mode its node body also says `ID from input` instead of displaying the unused fixed ID. These are display titles only: Set Global retains its `saved-value` and `previous-value` port IDs and process output keys, so existing wires and recordings require no migration. `Get Global` emits its `Variable ID` output even when `On Demand` returns a function value, and `Set Global` resolves the previous value from the same fixed or dynamic ID that it is about to write. New Get Global nodes default to `wait: true` and `onDemand: false`, and their editor metadata makes those two toggles mutually exclusive in the UI.
...
- determines split count from array-valued inputs and `splitRunMax`; an input
  definition can opt into `splitRunBehavior: 'preserve-array'` when an array is
  one configuration value rather than a fan-out dimension
