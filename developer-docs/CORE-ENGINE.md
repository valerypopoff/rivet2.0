...
The Delegate's persisted `message` output id is retained and displayed as `Tool Result Message`. Its persisted `assistant-message` output is displayed as `Message (fires before tool call invocation)` and carries nonblank assistant text emitted in the same model response as the tool calls. The output definition and runtime-object order is `assistant-message`, `tool-name`, `tool-arguments`, `output`, then `message`, so inline and fullscreen generic output rendering match the visible port order. `Tool Name` identifies the selected tool; `Tool Arguments` emits the normalized object actually passed to its handler. The pre-tool Message output is intrinsically per-call and pre-tool: every invocation emits the shared assistant text under its own process id, then starts that Message branch and its tool handler in parallel. A `Start Async Branch` boundary returns immediately and registers its remaining work with the root run when the branch must not hold the foreground path open. After each invocation's `nodeFinish`, its scalar Tool Name, Tool Arguments, Output, and Tool Result Message branches execute independently. Whitespace-only assistant text does not activate the pre-tool branch.
