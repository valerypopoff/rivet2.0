...
[`UiGraphNormalization.ts`](../packages/core/src/model/UiGraphNormalization.ts) is the single untrusted-data boundary for the persisted UI graph model. `normalizeUiGraph(...)` validates the graph envelope, every component discriminator and required field, every enum, and modern plus legacy `runGraph` action bindings. `normalizeUiGraphRecord(...)` additionally requires each graph's `id` to match its project-map key, and `normalizeProjectUiGraphs(...)` applies that contract to externally assembled project snapshots. Failures use `UiGraphNormalizationError.issues` with graph, component-index, binding-index, and field paths so file loaders and hosts can identify the exact malformed value. V4 serialization/deserialization, raw project validation, hosted editor snapshots, desktop rendering/action dispatch, and Node rendering/action dispatch all delegate to this parser; consumers must not add partial local shape checks.
...
Workflow-bound button consistency lives in [`UiGraphBindings.ts`](../packages/core/src/model/UiGraphBindings.ts). `reconcileProjectUiGraphButtonBindings(previousProject, nextProject)` is the immutable project-level operation used when workflow graph boundaries are persisted. It never matches rows by position: exact IDs win, and a data key crosses an ID change only when the same Graph Input/Output node proves the rename. Multiple mappings from one graph output to different UI data keys are valid fan-out and survive reconciliation. Automatic reconciliation touches only the boundary side whose IDs changed and preserves references for unrelated UI graphs. A button that covered the complete previous boundary gains deterministic rows for new ports; intentionally partial or empty mappings stay partial/empty so graph defaults and ignored outputs keep their runtime meaning. Already ambiguous blank/duplicate bindings are also left untouched so a background graph save cannot silently rewrite invalid configuration. `validateUiGraphButtonBindings(...)` and `validateProjectUiGraphButtonBindings(...)` are the non-mutating preflight APIs that report those configurations to desktop and hosted runners. Stale IDs, duplicate input targets, duplicate output state writes, and blank required data keys are blocking errors. Legacy literal input mappings remain literal during reconciliation.

`createUiGraphInteractionController(...)` is the shared mutable runtime for desktop and hosted renderers. It owns UI state, component-scoped errors, per-button loading, abort controllers, graph replacement cleanup, and subscriptions. Its action runner receives only the state mapped to that button plus an abort signal; both hosts therefore use the same single-flight, stale-write, cancellation, and error semantics. A button is single-flight, while different buttons may remain active independently. Each action claims its declared output state keys when it starts: disjoint patches can complete in any order, but for overlapping keys only the latest-started action may write. A direct input/textarea edit also claims its state key so an older action cannot overwrite newer user input. The lower-level `createUiGraphActionExecutionController(...)` remains the version/ownership primitive inside the interaction controller; renderers must not coordinate it themselves.
