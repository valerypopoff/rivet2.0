...
- `llmChatV2ProfileDataKeys` is the declarative ownership boundary for profile-controlled fields. Provider, model, credentials, generation parameters, provider reasoning/thinking controls, provider-native capabilities, headers, and extra provider options belong to the profile. Prompts, structured response format, Rivet tools and continuation, output switches, retry/diagnostics, editor caching, and OpenAI Previous Response ID remain invocation-local LLM Chat data.
