...
- JSON schema mode can opt into final `Response` object validation with **Fail the LLM profile on non-JSON response**. `chatV2Pipeline.ts` applies it only after `chatV2Outputs.ts` has assembled the exact parsed-or-string Data Value for the output port. A failure is not a transport retry: `llmProfileFallback.ts` records a distinct response-validation attempt and immediately advances to the next candidate, while Inline or scalar-profile execution throws the detailed validation error.
