...
`createUiGraphActionExecutionController(...)` is the shared action-lifecycle owner for desktop and hosted renderers. A button is single-flight, while different buttons may remain active independently. Each action claims its declared output state keys when it starts: disjoint patches can complete in any order, but for overlapping keys only the latest-started action may write. A direct input/textarea edit also claims its state key so an older action cannot overwrite newer user input. Execution IDs, rather than a single pending component ID, decide whether a completion may clear loading state or publish its component-scoped error. Resetting the controller invalidates every old completion without reusing execution IDs.
