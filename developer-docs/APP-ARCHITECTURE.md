...
`replaceCurrent`, `markCurrentProjectClean`, and `markProjectClean` so wrapper
shells can coordinate their own project list or file model without reaching into
Jotai atoms. The mark-clean methods are narrow post-save helpers for hosted
wrappers that need to re-seed the unsaved-changes baseline after a wrapper-owned
save succeeds; they update only Rivet's in-memory dirty/digest state and do not
save, reload, or switch projects. Open, replace, and close commands
