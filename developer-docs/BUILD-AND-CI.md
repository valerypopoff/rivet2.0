...
- `packages/app/scripts/prepare-tauri.mjs` syncs desktop version metadata from `packages/app/package.json` before rebuilding the app executor sidecar. Its child process environment strips a stale `NODE_OPTIONS` preload for `.pnp.cjs` when that loader file is absent, because some local checkouts can have Yarn PnP config plus a `node_modules` install layout; nested Node/Yarn sidecar builds must be allowed to start the pinned Yarn file instead of failing before Yarn runs.
