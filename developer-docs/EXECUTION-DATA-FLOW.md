...
`Message` is the nonblank model text
...
Library nodes are project-level definitions, not execution-history targets. When a graph contains a `nodePrefabInstance`, core resolves it to the current source node before preprocessing, but keeps the instance node id. The app should therefore store and render `nodeStart`, `nodeFinish`, output pages, selected output pages, recordings, run-to/run-from data, and Remote Debugger frames against the instance id in its containing graph. The library node should not receive execution data, because it can be linked from many graphs and has no graph execution context of its own.

This also means search, visual rendering, and settings display can use the resolved library node for title/ports/data, while execution-data lookup must stay keyed by the graph-local instance id. If a library node changes ports, app-side connection recovery updates each linked node's graph connections; existing execution history from older port shapes is ordinary stale run history and should be replaced by the next run rather than migrated.
...
- Retained Watch branch events keep their synthetic same-graph child `graphRunId` and their `parentGraphRunId`. The local editor, Remote Debugger transport, recording replay, restored project snapshots, and trace-created history entries retain that parent identity and project those bounded child processes into the selected owning graph run. This makes the first three and decisive/latest Watch iterations available as ordinary node-history pages without manufacturing a graph-run selector entry for each streamed update. The settled `streamingOutputWatchSummary.selectedIteration.graphRunId` identifies the one child page labelled **Terminal**. It is emitted only after the Watch settles—source completion, accepted Stop, or a terminal Watch failure; a running branch's current/latest page stays numbered. Replay remaps that child identity with the rest of the graph-run lineage, and active, remote, and restored snapshot projections apply the same root-scoped child-run marker rule. Before a repeated branch publishes the winning **Stop Watching Streaming Output** `nodeFinish`, Core atomically claims the Watch and marks that one event `streamingWatchTerminal: true`; parent continuation still waits until after the event is published. The marker survives Remote Debugger transport, recordings, replay, and snapshots, so inline and fullscreen Stop output can render the exact accepted `Value` even when a parallel loser finishes later. Stop has no pager. Child work from a different graph stays in its own graph view.
- A direct streaming producer-to-Graph Output connection uses a separate, in-process named-boundary callback when a Subgraph or Referenced Graph Alias is called. It relays only the Graph Output ID/value needed by an enclosing Watch, including across nested Subgraphs, Referenced Graph Aliases, and compiled Data Bus routes. It is intentionally **not** forwarded as a `partialOutput` event: otherwise every streamed increment would manufacture an additional caller node-history and recording row. The actual producer's ordinary partial events retain their existing lineage and visibility. Conditional Graph Outputs, split, duplicate, frozen, and Error-output boundaries are final-only because their eventual caller-visible value can differ from a child partial. A conditional **caller**, however, can forward named outputs after its condition passes; only early input-stream startup remains disallowed for that caller. Share these distinct eligibility rules with wire presentation, and mark a wire only after the complete route reaches a partial producer so final-only boundaries, unresolved recursive callers, ambiguous caller inputs, and shadowed Graph Output providers cannot leave misleading outer arrows. Dynamic graph callers with aggregate output contracts do not relay child Graph Output partials by this route; **Loop Until** may still publish its own per-iteration partials through its ordinary output contract.
...
recorder, and replayed with its original recording timestamp and remapped child
...
### Agent response trace events

`ProcessEvents.llmCallFinished` and `ProcessEvents.toolCallFinished` are
metadata-only observability events. They are forwarded through subprocessors,
Browser and Node execution, Remote Debugger transport, recording, and replay by
the same execution identity as the work they describe. Tool events cover
...
report omitted-row counts. Trace correlation must use root/graph/node/process
...
the events remain valid and produce an unavailable/partial inspector.
