...
Core owns workflow-bound button consistency through `reconcileProjectUiGraphButtonBindings(...)`, `validateUiGraphButtonBindings(...)`, and `validateProjectUiGraphButtonBindings(...)`. The app invokes reconciliation when a changed workflow boundary is committed into project state; mounting the builder does not rewrite mappings. Hosts can run project preflight before publishing without mutating their loaded revision. Node action execution rejects stale or ambiguous mappings with `RivetWebAppActionHttpError` code `invalid_button_bindings`; omitted mappings remain valid for graph defaults and intentionally ignored outputs.

[`UiGraphRuntimeModel.ts`](../packages/core/src/model/UiGraphRuntimeModel.ts) is the shared semantic owner for web-app component labels, input values, output formatting, JSON download eligibility and filenames, and action state patches. [`RivetWebAppRenderer.tsx`](../packages/app/src/components/rivetWebApps/RivetWebAppRenderer.tsx) and [`webAppClient.ts`](../packages/node/src/webAppClient.ts) consume that model while owning only React/direct-DOM adaptation such as focus, events, element construction, and Markdown integration; browser clipboard fallback and downloads are shared through [`UiGraphBrowserRuntime.ts`](../packages/core/src/model/UiGraphBrowserRuntime.ts). Cross-package client code imports this contract through the narrow `@valerypopoff/rivet2-core/web-app-runtime` package entrypoint, not through `packages/core/src`; the package emits dedicated ESM and CommonJS artifacts for that subpath. [`build-web-app-client.cjs`](../packages/node/scripts/build-web-app-client.cjs) aliases the public entrypoint to [`webAppRuntime.ts`](../packages/core/src/webAppRuntime.ts) while generating, so the checked-in browser artifact is built from current source even before package output exists. The generator is a true CommonJS launcher so esbuild stays on Yarn PnP's synchronous CJS resolution path under Node 22 Linux. The hosted client is authored as TypeScript and generated by that script, then injected by [`webAppHandler.ts`](../packages/node/src/webAppHandler.ts); do not restore a handwritten `String.raw` browser program. The Node build and root style check both verify that the checked-in generated client is fresh.

The runtime model exports `createUiGraphInteractionController(...)` as the shared mutable interaction owner for React preview and the generated hosted client. It wraps the lower-level `createUiGraphActionExecutionController(...)` and owns local state, per-button loading and errors, abort controllers, graph replacement cleanup, and stale-patch protection. Renderers subscribe to the interaction controller and must not keep parallel pending-action maps or call the lower-level execution controller directly. Direct form edits supersede older action writes to the same key, while distinct buttons may run independently. Hosted actions abort outstanding requests on page unload or revision mismatch. `runRivetWebAppAction(...)` uses `createProcessorOptions.abortSignal` when supplied, otherwise it forwards the provided Fetch `Request.signal` through an action-scoped signal to `createProcessor(...)`; its forwarding listener is removed when the action settles so long-lived host signals do not retain completed processors. Express adapters that synthesize a Fetch `Request` should connect that signal to the underlying response/request close event if they want client disconnects to cancel graph execution.
