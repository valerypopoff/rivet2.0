...
The runtime model also exports `createUiGraphActionExecutionController(...)`. Both renderers use its monotonic execution IDs for per-button loading and latest-started ownership of overlapping output state keys; direct form edits supersede older action writes to the same key. Do not replace it with a `Set<UiComponentId>`: a set fixes the spinner but cannot prevent stale state patches or same-button completion races. Hosted actions attach an `AbortController` to each fetch and abort outstanding requests on page unload or revision mismatch. `runRivetWebAppAction(...)` uses `createProcessorOptions.abortSignal` when supplied, otherwise it forwards the provided Fetch `Request.signal` through an action-scoped signal to `createProcessor(...)`; its forwarding listener is removed when the action settles so long-lived host signals do not retain completed processors. Express adapters that synthesize a Fetch `Request` should connect that signal to the underlying response/request close event if they want client disconnects to cancel graph execution.
