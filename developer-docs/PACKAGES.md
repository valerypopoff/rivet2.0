...
| Packages | Version | Changes |
| --- | --- | --- |
| Core, Node, CLI, Evaluations | `2.12.0` → `2.13.0` | Core adds Project Name, optional empty-prompt filtering, corrected Match case output labels, and live streamed inputs through nested Subgraphs with bounded Watch history and hardened lifecycle handling. Node, CLI, and Evaluations have no package-local changes; they are required public npm companions. |
| App | `2.18.0` → `2.19.0` | Adds Project Name presentation and empty-prompt filtering summaries, restores saved graph viewports, completes the searchable inline Subgraph selector, and shows streaming arrows through nested graph boundaries. |
| Docs | `2.10.0` → `2.11.0` | Documents Project Name, empty-prompt filtering, Match case labels, and streaming through Subgraph inputs with final-only boundaries and bounded history. |
| All five Studio Server packages | `1.17.0` → `1.18.0` | Web fixes project reopening/focus for viewport restoration and adds browser coverage for the editor and node features above. API, bootstrap, executor, and shared have no package-local changes; they are required Studio Server companions. |
| App Executor | `2.6.0` (unchanged) | No package-local changes. |

Minor releases cover the new node and streaming capabilities. Desktop Tauri/Cargo
metadata and the CLI Docker fallback are synchronized. Public npm and Studio
Server lockstep policies remain in force; Helm chart versions are independent.
The migration-ledger repair is deployment tooling, not a separately versioned
package. No dependency or Yarn lockfile update is required.
...
- Root forwarding command: `yarn docs <script>` (for example, `yarn docs dev`)
...
- GitHub Pages deployment at `/rivet2.0/`, with a Docusaurus Pages promotional homepage at `/`, the User Guide introduction at `/user-guide`, the remaining documentation routes kept at their existing paths, and a top-right `/download` page that reads stable Windows/macOS GitHub Release-feed metadata generated by the main-branch workflow plus developer Windows/macOS metadata generated by the develop-branch workflow
- `packages/docs/src/pages/index.tsx` owns the promotional homepage structure, `src/content/homepageContent.ts` centralizes its editable copy and links outside Docusaurus's route-generating `src/pages` directory, `src/pages/index.module.css` keeps its responsive presentation isolated from documentation articles, and `static/img/social-card-v2.png` carries the matching link-preview artwork configured in `docusaurus.config.js`. The hero presents three horizontal, selectable use-case summaries above one live Rivet window; the Foundations and Use Cases sections each own another live window. Every window loads a bundled project through the shared same-origin promo app, stays compact in the page, and enlarges the same iframe for its popup view. The docs plugin still uses `routeBasePath: '/'`; do not move the full documentation tree under another prefix merely to preserve the dedicated root page. The docs shell and landing presentation mirror the app's canonical visual modes through a deliberately small token subset: Roboto and Roboto Mono bundled through Fontsource, Molten colors in Docusaurus dark mode, and Bright colors in light mode. Keep those docs-owned mappings in `src/css/custom.css` and `src/pages/index.module.css`; importing the app's global CSS would also import editor resets and component contracts that do not belong in Docusaurus. Bundling the faces keeps the rendered typography stable when a browser, privacy setting, or offline deployment blocks third-party font requests.
- Canonical docs development builds the promo app into the ignored `.promo-dev/rivet-demo` directory and serves it through the one Docusaurus process. Do not restore a Vite Preview companion server: a separately owned port can outlive the docs launcher and serve stale HTML or mismatched assets. `dev:promo` remains only as a source-mode convenience for working on the promo host itself; it is not the landing-page development topology. The app-local promo validator owns project topology and provider-free fixture execution, while the root `check:promo-catalog` command owns cross-package parity between `promoProjectManifest.ts` and the section placements in `homepageContent.ts`; keep that docs import out of the app workspace build. The production bundle check resolves initial assets and verifies every emitted Vite-native Monaco worker, guarding against a publish layout that omits lazy language workers.
