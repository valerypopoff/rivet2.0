...
- `packages/docs/src/pages/index.tsx` owns the promotional homepage structure, `src/content/homepageContent.ts` centralizes its editable copy and links outside Docusaurus's route-generating `src/pages` directory, `src/pages/index.module.css` keeps its responsive presentation isolated from documentation articles, and `static/img/social-card-v2.png` carries the matching link-preview artwork configured in `docusaurus.config.js`. The hero keeps introductory copy above its showcase at every width; on wide layouts, concise feature explanations sit left of the workflow window, then stack above it when space narrows. The docs plugin still uses `routeBasePath: '/'`; do not move the full documentation tree under another prefix merely to preserve the dedicated root page. The docs shell and landing presentation mirror the app's canonical visual modes through a deliberately small token subset: Roboto and Roboto Mono bundled through Fontsource, Molten colors in Docusaurus dark mode, and Bright colors in light mode. Keep those docs-owned mappings in `src/css/custom.css` and `src/pages/index.module.css`; importing the app's global CSS would also import editor resets and component contracts that do not belong in Docusaurus. Bundling the faces keeps the rendered typography stable when a browser, privacy setting, or offline deployment blocks third-party font requests.
- The workflow illustration is a responsive composition, not a screenshot: node body and header share the same radius contract, and the visible port-to-port links are SVG Bezier paths measured from the actual port elements after rendering. Keep the measurement and link generation in `WorkflowPreview` and the responsive layout values in `index.module.css`; do not replace those paths with approximate straight divs or fixed coordinates.
