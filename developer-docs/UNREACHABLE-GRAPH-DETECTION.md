...
| `DelegateFunctionCallNode` auto delegate                | Runtime name match  | `direct-static`            | Yes            | Picks the first graph whose name contains the function name | No             | Counts only static-name `Tool` (`gptFunction`) nodes with an enabled connection path to the runtime-selected delegate input; dynamic Tool names are not provable |
...
Tool node with a stored name. Dynamic Tool names remain unproven and do not affect the indicator.
