...
The server emits one invalidation only after a successful create, move, rename, delete, upload, duplicate, restore, save, publish/unpublish, or web-app publication/access mutation. Rejected and failed operations emit nothing. Each loaded dashboard document includes an opaque, in-memory client id in the `x-rivet-workflow-tree-client` request header; it is a refresh-deduplication token, not an authentication credential. The API reflects that id only in the event for the originating mutation, so the browser's existing local refresh remains authoritative and it does not schedule a redundant stream refresh for its own change. The id is not kept in `sessionStorage`: a tab opened with an opener may inherit a copy of that storage and must not suppress another tab's changes. Reloading creates a new id and obtains the current tree and stream state afresh.
...
### Project conflict freshness and notification ownership

Content reconciliation starts with an editor bridge capture **before** fetching the tree. The capture identifies the current iframe instance, an observation sequence, and the generation of each open editable project. The editor claims each observation once before applying its revision or structural binding. A local save beginning or settling, loading/reloading, accepting a conflict, restoring a failed reload, or closing/rebinding the project invalidates older observations. Ordinary unsaved graph edits do not. Revisions remain opaque: an earlier revision may legitimately return after an intentional server restore.

The hosted IO provider fences the complete save preparation/request lifetime and always releases that fence in `finally`, including rejected requests and HTTP 409 conflicts. Projects currently saving or reloading are deferred independently of other open projects. Settlement requests a new capture and tree fetch. Remote reconciliation stays single-flight and coalesces retries; transport/bridge failures retry after the existing delay, while an active local operation wakes reconciliation when it settles. The delayed post-save sidebar refresh is only presentation, not a freshness guarantee.

The revision tracker owns conflict state. A valid observation matching the accepted revision clears an obsolete pending conflict; a repeated pending revision retains its conflict identity. Path rebinding does not authorize a differing saved revision: combined rename/move and content changes remain protected. The editor sends complete, versioned conflict snapshots to the dashboard. Every snapshot publication receives a new sequence, including presentation-only title updates that leave the conflict's `changeId` intact; the notice updates its displayed project without restarting the resolution action. A path move invalidates the old resolution identity and publishes the warning at the new path. Only snapshots from the current iframe instance with increasing sequence numbers can create, update, or dismiss warnings. A bridge timeout is an explicit retry outcome, never an authoritative empty list. Content and structural notices use neutral wording: the originating tree notification's browser ID does not establish who changed a particular project.

Reload and Keep mine carry the displayed conflict's opaque `changeId` as well as its project ID, canonical path, and revision. A stale decision cannot authorize a different conflict, even if a revision value later repeats. Reload keeps the warning and blocks saves while replacement is provisional; failure restores the original revision decision with a new runtime generation. Successful resolution triggers another reconciliation. Asynchronous button callbacks never dismiss a toast; the next authoritative snapshot owns dismissal, preventing an older callback from closing a newer warning. The server still checks the expected revision immediately before overwriting content.

Regression coverage lives in the hosted revision-tracker, conflict-notice, and editor-bridge tests (`yarn workspace @valerypopoff/rivet-studio-server-web test`). Run `PLAYWRIGHT_HEADLESS=1 PLAYWRIGHT_SLOW_MO=0 yarn studio-server:ui:observe workflow-tree-sync.spec.ts` for controlled tree/save response races and real dashboard/editor notification behavior. Hosted-project API tests retain filesystem and managed compare-and-swap coverage. No new storage schema or deployment setting is involved.

The browser suite forwards API traffic to a disposable server and uses the checkout's hosted UI (`PLAYWRIGHT_BASE_URL` may point to its Vite server). It controls response release and observes bridge/save completion, rather than assuming a delay proves a race. The two-browser tree-refresh fixture finishes both initial tree responses before mutating its shared tree, so a first load cannot masquerade as a remote refresh. A separate opener-cloned-tab fixture proves a second tab does not inherit the first dashboard's event-suppression identity. Coverage includes an old tree arriving after the save acknowledgement, a tree explicitly deferred while the save response is held, genuine remote edits (including a move with a differing revision), rejected saves without an event, failed reloads/fetches, late resolution acknowledgements, inactive tabs, iframe replacement, and old snapshot delivery after closing/reopening. A separate pair of browser request contexts exercises real API revision rejection: the stale writer receives 409 and the winning revision remains stored. This fixture does not use or modify an operator's existing workflows.
...
WebSocket authorization has two layers. Every frame checks the opening session's credentials, and the action-start, cancel, and resume commands additionally read the **current** app binding, its slug/UI graph ownership, and OAuth allowlist before they affect work. Removing a user's email, unpublishing an app, or rebinding its slug therefore prevents an old page from starting new work, even while its socket remains open. The same publication change does not disturb another socket whose user still satisfies the updated allowlist. A code-only republish of the same UI graph deliberately keeps the app binding: the socket remains on its pinned graph revision rather than being silently switched to new code. Filesystem settings retain an opaque binding ID; managed storage uses `workflow_web_apps.app_id`. A pre-binding filesystem sidecar receives an opaque ID when that individual app is next published or its access policy is edited, which intentionally invalidates any socket still using its temporary legacy binding. A filesystem publication mutation emits a targeted local invalidation; managed storage does the same after commit and from its PostgreSQL invalidation listener. A process owns one five-second recheck timer for all idle sockets, and the bounded lookup coordinator shares each app-binding read. A failed policy check closes only the affected socket with retryable code `1013`; an unexpected callback failure cannot stop the other sockets' checks. The recheck remains the fail-closed fallback for a missed notification.

The policy read is intentionally small: filesystem storage rereads only the owning project's settings sidecar, and managed storage queries only the current `workflow_web_apps` row. Background checks for one app share one bounded lookup; action commands use a fresh lookup. If a targeted invalidation arrives while a command's lookup is in flight, the command retries once against the newer policy generation; another concurrent change fails closed instead of allowing a stale read or retrying indefinitely. The runtime admits at most 16 concurrent policy reads and gives each five seconds. It has no waiting queue: a saturated or slow policy source closes the affected socket with code `1013` so the browser can retry, rather than preserving stale access.

If a policy changes after an action was accepted, the pinned graph revision is allowed to finish, but the revoked page is immediately detached from that action's progress and terminal-result delivery. Its existing browser-storage RPC frames remain available only so that accepted action can finish safely. A newly attempted start, cancel, or resume is rejected with `access_revoked`. Once that socket has no accepted action left, the server closes it with WebSocket code `1008`; an idle revoked socket closes immediately. Losing the credentials that opened the socket also closes it immediately. The gateway checks that delivery boundary again after every asynchronous durable read and remote-subscription step, and requires the same gateway connection to remain live. Remote subscriptions carry their own attachment identity, so a late callback from a superseded resume cannot remove a newer subscription for the same run. A failed remote attachment is disposed before its ordinary `1011` transport close; after policy revocation, the same failure is reported but cannot preempt an active accepted action's browser-storage exchange. Thus, a replay or catch-up that was already waiting when access changed cannot leak an old progress or terminal event, and a delayed read cannot recreate a subscription for a disconnected socket. This deliberately distinguishes execution consistency from current authorization: revocation never replaces a graph under a running action, and it never lets the revoked browser observe or control more work.

The managed Kubernetes release gate verifies this boundary with real separate control-plane and replicated execution API processes sharing its disposable PostgreSQL instance. It enables a loopback-only dummy OAuth provider **only inside that isolated gate**, completes the normal dummy OAuth login flow for a removed user and a retained user, opens their sockets, changes the allowlist through the control-plane API, then requires the removed socket to close with `1008`, rejects that user's fresh HTTP action, and confirms the already-open retained socket can still run. The gate restores its normal unauthenticated disposable setting afterward. This is a release regression fixture, never a production auth configuration.

The web-app access update endpoint is selective: it changes only the app bindings included in the request. It must preserve the allowlists of other published apps in the same project; treating an omitted app as an empty allowlist could silently revoke unrelated users and their sockets.
...
The workflow summary and ordinary run pages are metadata-only reads. They do not read compressed recording/replay payloads, and the workflow summary omits project graph/node statistics and aggregate web-app publication status that the recordings modal does not display. Full recording or replay artifacts are loaded only when the user opens a run; input filtering reads recording artifacts incrementally because the predicate depends on captured input data. It fetches only a bounded newest-first metadata window, probes the newest artifact before concurrent older candidates, and continues through a cursor instead of loading a workflow's complete history into the API process.
...
- supports an optional input filter. The filter uses a JSON path where `$` is the root graph input value recorded under Rivet's `inputs.input.value`; for workflow endpoint runs that value is the HTTP request body, and for web-app action runs with an `input` graph port it is the UI state mapped to that port. If a recorded run has graph inputs but no `input` port, `$` falls back to an object of all captured graph input values keyed by port name. For example, request input `{ "foo": "bar" }` matches `$.foo == bar`. Filtered searches scan newest-first, check a newest run by itself before older candidates, append matches to the visible list as each cursor response returns, and keep searching automatically until the history is exhausted or the user clicks `Stop search`. The filter panel shows an accessible progress bar with the status-scoped available-run count, server-reported analyzed candidate count, and percentage. This adds no count query or recording read; a completed search is 100%, while a stopped search retains its last partial value. Clearing or hiding the filter uses the same stop path and aborts the in-flight request.
...
Input filtering does not change how recordings are created. When `inputPath` is present, the API reads existing recording artifacts after the workflow/status filter, newest first, extracts the recorded root graph input value from the `start` or `graphStart` event, restores Rivet string-table references only inside that input, and applies the JSON-path/operator/value predicate. It never reconstructs all recording events or assets merely to compare the request. The extractor keeps endpoint compatibility by preferring `inputs.input.value`; when a web-app action or other graph run records only named input ports, it searches an object of all captured input values instead. The API fetches at most a bounded metadata candidate window per cursor request and, for the initial cursor, reads the single newest artifact first. A short-lived process-local cache reuses valid extracted inputs for repeated searches; it is keyed by a managed storage owner plus immutable blob key, or by a filesystem artifact's path and stat identity, retains only the small extracted input, expires after five minutes, and is only an optimization—recording artifacts remain the source of truth. Malformed artifacts are not cached. Cold artifact extraction is also capped process-wide, so parallel browser searches cannot fan out unbounded decompression or object-store reads. The API transfers filesystem and built-in managed-store bytes into that bounded worker, where decompression, decoding, JSON parsing, and extraction happen off the API event loop. A normal filesystem deletion race is skipped, but an unexpected filesystem or object-store read error fails the request rather than being misreported as a non-match. The response may return before the full history has been exhausted, even when the current scan window has no matches; in that case `totalRunsExact` is `false`, `hasMore` is `true`, and `nextInputCursor` can be passed back as `inputCursor` to continue searching older recordings. The dashboard uses that cursor automatically, appending each newly found run to the same filtered list and showing a searching/completed/stopped status. This keeps old recordings readable, avoids adding wrapper-specific fields to the recording write path, and makes recent-request lookups responsive even when historical artifacts live in object storage. The browser request is abortable: the API waits only for the current small read batch, removes any of that search's cold reads still queued behind the process-wide cap, and stops when the user closes the modal, stops the search, clears or hides the filter, or navigates away. Supported operators are `==`, `!=`, `>`, `>=`, `<`, `<=`, `contains`, `exists`, and `not_exists`. When `contains` receives a filter value that parses as a string, including single-quoted text such as `'request_id'`, the resolved left operand is treated as a string too; objects and arrays are JSON-stringified, so `$ contains 'request_id'` searches the whole recorded input object. A missing JSON path matches `not_exists`, does not match `exists`, and resolves to actual `undefined` for the other operators; the filter value literal `undefined` also parses as `undefined`. Ordering comparisons with `undefined` do not match.
