...
`managed-reconciliation:status` is a read-only managed-PostgreSQL operator command. It reports per-domain phase, completed generation, error timestamps, and aggregate finding counts; it intentionally never prints object keys, prompts, run payloads, or credentials. Run it from a control-plane environment with the same managed-storage configuration and database credentials as the API.
