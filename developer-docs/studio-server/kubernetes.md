...
The migration Job bootstraps bucket, endpoint, region, prefix, and path-style as separate fields in the encrypted deployment-storage settings row. The legacy combined `storageUrl` remains in the version-1 payload for older binaries, but new binaries do not infer a custom provider's region or the workflow prefix from it. The chart passes `objectStorage.prefix` into the bootstrap environment. Endpoint must be an HTTP(S) origin; prefix must be a relative path ending in `/` and cannot overlap `runtime-libraries/`. After Vault dotenv loading, Kubernetes restores chart-owned storage location, PostgreSQL target and pool limit, storage/database modes, app-settings backend, topology, and app-data root; Vault can still supply credentials such as the database password and S3 keys. A stale Vault `RIVET_DEPLOYMENT_STORAGE_URL` alongside the chart bucket fails bootstrap instead of silently redirecting older readers. Immediately after importing the authoritative PostgreSQL row, the migration Job runs the same redacted storage-location comparison as pod startup, so a mismatch is visible before rollout. It names differing fields but never prints credentials or overwrites the row. Pod-local projection also refuses to start a Kubernetes pod if the authoritative row selects filesystem storage or local-Docker PostgreSQL.
