...
| `yarn studio-server:dev` / `yarn studio-server:dev:docker`                                                                                                                             | Starts the Docker dev stack and refreshes an already-running dev proxy                                                                                                                          | Closest-to-production browser testing                                                                               |
...
- `yarn studio-server:prod` (and its explicit `yarn studio-server:prod:prebuilt` alias) pulls prebuilt images under `ghcr.io/valerypopoff/cloud-hosted-rivet2-wrapper/{proxy,web,api,executor}:${RIVET_IMAGE_TAG:-latest}`, then force-recreates the stack with `--no-build`; set `RIVET_PROXY_IMAGE`, `RIVET_WEB_IMAGE`, `RIVET_API_IMAGE`, or `RIVET_EXECUTOR_IMAGE` to pin any service to a different image. Keep the image examples in `deploy/studio-server/.env.example` on that same namespace.
...
- The image-build workflow runs that complete `yarn studio-server:test` gate against the exact monorepo commit before the image matrix. Image jobs then publish a deterministic staging tag keyed by that commit. A final promotion job applies `latest`, branch, release-tag, and short-SHA tags only after all four images build and the managed-Kubernetes release gate succeeds, so an image-build or runtime-gate failure cannot leave the public tag set half-updated. Runs for the same Git ref are serialized, transient build/push failures receive one cache-backed retry, registry promotion receives bounded retries, and third-party actions are pinned to reviewed commit SHAs. GHCR cannot promote four repositories as one transaction, so rerun a workflow if a persistent registry outage interrupts promotion.
