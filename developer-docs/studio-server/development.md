...
| `yarn studio-server:dev` | Starts the Docker dev stack and refreshes an already-running dev proxy | Closest-to-production browser testing |
| `yarn studio-server:dev:docker:recreate` | Rebuilds and recreates the Docker dev stack | Pick up Dockerfile/env/runtime changes |
...
| `yarn studio-server:dev:docker:down` | Stops the Docker dev stack | Cleanup |
...
| `yarn studio-server:test` | Builds Studio Server dependencies/workspaces, runs API and pure web tests, and executes test-style, repo-structure, and Kubernetes contracts | One-command pre-commit or branch verification |
...
| `yarn studio-server:verify:kubernetes` | Runs Kubernetes launcher/chart contract tests, renders the local rehearsal values path, and lint-renders the production overlay | Catch local/prod chart drift before handing the repo to operators |
...
- `yarn studio-server:dev` / `yarn studio-server:dev:docker:*` use `deploy/studio-server/compose/docker-compose.managed-services.yml` plus `deploy/studio-server/compose/docker-compose.dev.yml`
- `yarn studio-server:prod`, `yarn studio-server:prod:restart`, and `yarn studio-server:prod:custom` use `deploy/studio-server/compose/docker-compose.managed-services.yml` plus `deploy/studio-server/compose/docker-compose.yml`
...
- `yarn studio-server:prod` pulls prebuilt images under `ghcr.io/valerypopoff/cloud-hosted-rivet2-wrapper/{proxy,web,api,executor}:${RIVET_IMAGE_TAG:-latest}`, then force-recreates the stack with `--no-build`; set `RIVET_PROXY_IMAGE`, `RIVET_WEB_IMAGE`, `RIVET_API_IMAGE`, or `RIVET_EXECUTOR_IMAGE` to pin any service to a different image. Keep the image examples in `.env.example` on that same namespace.
...
- `yarn studio-server:dev` refreshes only the already-running dev `proxy` service after the stack is up, so nginx re-reads mounted route templates without forcing a full API/web/executor recreate. The launcher also recreates a running dev `web` or `api` service when its dependency marker no longer matches the root lockfile. Saved public-route changes normally apply through the proxy watcher without rerunning the launcher. If a new proxied route such as `${RIVET_LATEST_APPS_BASE_PATH:-/apps-latest}` falls through to the Studio UI, check the proxy logs for a failed nginx reload before using `yarn studio-server:dev:docker:recreate` for a full reset.
...
- the dev stack keeps container dependency state in Docker named volumes and keys its freshness marker to the root Yarn metadata. The live-mounted web and API services set `YARN_NODE_LINKER=node-modules` for their entire process lifetime, so post-install workspace commands use the same layout as the mounted dependency volume. They also put `YARN_INSTALL_STATE_PATH` inside that volume; a container install must never overwrite the host checkout's PnP install state. Do not reuse host-native unplugged artifacts inside Linux containers. The launchers use separate `rivet-studio-server-dev` and `rivet-studio-server-prod` Compose project names, so containers and named volumes from one mode cannot contaminate the other.
...
- The image-build workflow runs the cheap repository guardrails (`yarn studio-server:verify:repo-structure` and `yarn studio-server:verify:test-style`) once before the image matrix. Image jobs publish a deterministic staging tag keyed by the monorepo commit. A final promotion job applies `latest`, branch, release-tag, and short-SHA tags only after all four images build successfully, so an image-build failure cannot leave the public tag set half-updated. Runs for the same Git ref are serialized, transient build/push failures receive one cache-backed retry, registry promotion receives bounded retries, and third-party actions are pinned to reviewed commit SHAs. GHCR cannot promote four repositories as one transaction, so rerun a workflow if a persistent registry outage interrupts promotion. Full API tests remain developer/compatibility gates rather than work duplicated in each image job.
...
- To run only specific API test files, use `yarn workspace @valerypopoff/rivet-studio-server-api run test:files -- src/tests/example.test.ts`. Do not pass file names to `yarn workspace @valerypopoff/rivet-studio-server-api run test -- ...`; the default API script is an explicit manifest and would still run the full suite before appending the extra files.
...
- `deploy/studio-server/scripts/update-check.sh` must list every active `createModuleOverrideAliases(...)` target. `yarn studio-server:verify:web-pure` checks that the scanner and Vite aliases stay aligned, so update both when adding or removing hosted upstream overrides.
- `yarn studio-server:verify:kubernetes` lint-renders the Helm chart with real image repository overrides and verifies the key negative cases:
...
1. remember that `yarn studio-server:prod` and `yarn studio-server:prod` use pulled images, `yarn studio-server:prod:restart` keeps already-local images, and `yarn studio-server:prod:custom` uses your current workspace and `rivet/` folder
