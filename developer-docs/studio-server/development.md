...
the separately deployed hosted executor contains the changed engine.
...
- Docker dev source-mounts the repository into the editor's Node executor and runs the executor-specific esbuild watcher. The watcher bundles Core and Node directly from their workspace source entrypoints, watches the complete transitive bundle graph, and replaces only the child executor process after each successful rebuild. Core, Node, app-executor, studio-server-executor, and shared-contract edits therefore reach subsequent hosted Node runs without restarting `yarn studio-server:dev`, rebuilding host package outputs, or recreating the container. A failed rebuild keeps the dev supervisor alive but does not replace the last successfully built child; fixing the source triggers another build and replacement. The executor uses its own named `node_modules` and Yarn-unplugged volumes so dependency installation cannot overwrite the host's PnP state or race the API/web dependency volumes. Lockfile changes still require the launcher's normal whole-stack dependency reconciliation.
