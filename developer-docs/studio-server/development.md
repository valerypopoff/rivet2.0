...
| `yarn studio-server:test`                                                                                                                                                              | Verifies the migration ledger, cleanup safety contract, builds Studio Server dependencies/workspaces, runs API and pure web tests, and executes host-compatibility, test-style, repo-structure, and Kubernetes contracts | One-command pre-commit or branch verification                                                                       |
...
- Docker dev does **not** source-mount or watch the editor's Node executor. It runs `/app/executor-bundle.cjs` from the last built executor image, independently of the source-mounted Vite web and API services. Changes to Core, Node, app-executor, or studio-server-executor therefore require an executor image rebuild and container replacement before hosted Node runs use them. The executor bundler resolves the Core and Node workspace entries directly from source; rebuilding host ESM or CJS exports alone, refreshing the browser, or rerunning the idempotent `studio-server:dev` command cannot update an existing executor image. Use the documented Docker recreation workflow for the intended stack, and verify the actual hosted Node WebSocket path as well as Browser behavior for cross-runtime changes.
