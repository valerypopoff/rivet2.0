...
- `Settings` -> `General` -> `Trusted clients` lists verified client IPs/networks that bypass the server UI gate, web-app auth, and public workflow bearer checks. Hostnames never authorize access; see [Trusted clients](trusted-clients.md).
...
Keep any diagnostic API port, such as the Compose `http://localhost:3100` binding, private to the host or trusted operators. Public browser traffic should enter through the Rivet proxy so route auth, verified client-address handoffs, forwarded host/protocol normalization, UI cookies, and web-app OAuth behavior all share one boundary.
...
The wrapper's legacy HTTP action adapter also preserves upstream web-app stored-value semantics: it validates the browser's `storage` snapshot, supplies a per-action snapshot store to `createProcessor`, and returns the resulting `storagePatch`. WebSocket actions use the same behavior through upstream `prepareRivetWebAppAction`. This browser-scoped storage is separate from project datasets and is never treated as trusted server-side state.
...
- verified clients listed in `Settings` -> `General` -> `Trusted clients` bypass web-app auth for both web-app route families too
- workflow endpoint routes remain bearer/trusted-client routes and do not accept UI or OAuth web-app session cookies as a substitute for `Authorization`
...
- verified client IPs/networks listed in `Settings` -> `General` -> `Trusted clients` bypass public workflow bearer auth; the API checks the authenticated `X-Rivet-Client-IP` handoff, never the retired hostname flag
...
`yarn studio-server:dev` preserves the nginx routing and auth model described above.

`yarn studio-server:dev:local` does not recreate that proxy boundary. It starts the services directly and serves the web app from Vite on `http://localhost:5174`.
