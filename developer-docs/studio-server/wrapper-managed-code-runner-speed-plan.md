...
```bash
yarn workspace @valerypopoff/rivet-studio-server-api run workflow-execution:measure -- \
...
```bash
yarn workspace @valerypopoff/rivet-studio-server-api run workflow-execution:measure -- --base-url <url> --endpoint <endpoint> --kind published --runs 50 --warmups 10 --body '<json>'
```
...
yarn workspace @valerypopoff/rivet-studio-server-api run workflow-execution:measure -- --base-url http://localhost:8080 --endpoint graph-fixture-speed --kind published --runs 50 --warmups 10
