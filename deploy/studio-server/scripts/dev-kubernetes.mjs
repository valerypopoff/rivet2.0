...
function getChartName() {
  const chartYaml = fs.readFileSync(path.join(rootDir, 'deploy', 'studio-server', 'helm', 'Chart.yaml'), 'utf8');
  const match = chartYaml.match(/^name:\s*(.+)$/m);
  if (!match) {
    throw new Error(`[${launcherName}] Could not determine chart name from deploy/studio-server/helm/Chart.yaml`);
  }
...
async function buildImages(dockerBin, config, env) {
  const buildSpecs = [
    { dockerfile: 'deploy/studio-server/images/api/Dockerfile', image: config.images.api },
    { dockerfile: 'deploy/studio-server/images/executor/Dockerfile', image: config.images.executor },
    { dockerfile: 'deploy/studio-server/images/web/Dockerfile', image: config.images.web },
    { dockerfile: 'deploy/studio-server/images/proxy/Dockerfile', image: config.images.proxy },
  ];
...
async function main() {
  ...
  switch (action) {
    ...
    default:
      ...
      console.error('Usage: yarn studio-server:dev:kubernetes-test[:recreate|:build|:up|:down|:config|:ps|:logs]');
      process.exit(1);
  }
}
