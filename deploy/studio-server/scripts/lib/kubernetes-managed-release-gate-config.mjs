...
function parseImagePullPolicy(env) {
  const name = 'RIVET_K8S_RELEASE_GATE_IMAGE_PULL_POLICY';
  const value = optionalString(env, name, 'Always');
  if (!['Always', 'IfNotPresent'].includes(value)) {
    throw new Error(`[kubernetes-managed-release-gate] ${name} must be Always or IfNotPresent`);
  }
  return value;
}
