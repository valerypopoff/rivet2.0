...
const sampleProjectPath = path.join(rootDir, 'packages', 'node', 'test', 'test-graphs.rivet-project');
...
function readSampleProjectContents() {
  if (!fs.existsSync(sampleProjectPath)) {
    throw new Error(
      'Compatibility fixtures require packages/node/test/test-graphs.rivet-project. ' +
      'The monorepo checkout is incomplete.',
    );
  }
...
async function runDockerServicesWithEnv(envPath, launcherScript) {
  const env = {
    ...process.env,
    RIVET_ENV_FILE: envPath,
  };

  return await runCapture(`node ${launcherScript} services`, env);
}

async function runRepoLocalBaseline() {
  const env = createRepoLocalTestEnv();
  await run('yarn workspace @valerypopoff/rivet-studio-server-api run build', env);
  await run('yarn workspace @valerypopoff/rivet-studio-server-api run test', env);
}

async function runSplitRepoLocalChecks() {
  const env = createRepoLocalTestEnv();
  await run(
    'yarn workspace @valerypopoff/rivet-studio-server-api run test:files ' +
...
function assertDockerConfigExcludesManagedWorkflowServices(label, output) {
  ...
  if (services.has('workflow-postgres') || services.has('workflow-minio')) {
...
async function verifyFilesystemDocker() {
  ...
  try {
    ...
    const devServices = await runDockerServicesWithEnv(scenario.envPath, 'deploy/studio-server/scripts/dev-docker.mjs');
    const prodServices = await runDockerServicesWithEnv(scenario.envPath, 'deploy/studio-server/scripts/prod-docker.mjs');
    assertDockerConfigExcludesManagedWorkflowServices('dev-docker services', devServices.stdout);
    assertDockerConfigExcludesManagedWorkflowServices('prod-docker services', prodServices.stdout);
  } finally {
    scenario.cleanup();
  }
}
...
async function verifyLocalDocker() {
  ...
  try {
    ...
    const devServices = await runDockerServicesWithEnv(scenario.envPath, 'deploy/studio-server/scripts/dev-docker.mjs');
    const prodServices = await runDockerServicesWithEnv(scenario.envPath, 'deploy/studio-server/scripts/prod-docker.mjs');
    assertDockerConfigIncludesManagedWorkflowServices('dev-docker services', devServices.stdout);
    assertDockerConfigIncludesManagedWorkflowServices('prod-docker services', prodServices.stdout);
  } finally {
    scenario.cleanup();
  }
}
...
if (!action || !(action in actions)) {
  console.error('Usage: yarn studio-server:verify:{filesystem|filesystem:docker|local-docker|local-docker:split}');
  process.exit(1);
}
