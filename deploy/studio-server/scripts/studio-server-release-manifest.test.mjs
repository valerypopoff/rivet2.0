...
test('forward rollback retains the migrated schema and restores only a compatible image set', () => {
  ...
  assert.deepEqual(values.workflowSchema.compatibility, { minimumVersion: 2, maximumVersion: 8 });
  assert.equal(values.release.production.database.managedWorkflowSchemaVersion, 8);
