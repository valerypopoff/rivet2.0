...
test('only promoted manifests can produce production Helm values', () => {
  ...
  assert.deepEqual(values.workflowSchema.compatibility, { minimumVersion: 3, maximumVersion: 3 });
...
test('forward rollback retains the migrated schema and restores only a compatible image set', () => {
  ...
  const failedRelease = assertStudioServerReleaseManifest(
    {
      ...
      database: {
        managedWorkflowSchema: {
          version: 4,
          minimumRollbackCompatibleVersion: 3,
        },
      },
    },
  ...
  assert.deepEqual(values.workflowSchema.compatibility, { minimumVersion: 2, maximumVersion: 4 });
  assert.equal(values.release.production.database.managedWorkflowSchemaVersion, 4);
  ...
  const failedRelease = assertStudioServerReleaseManifest(
    {
      ...
      database: {
        managedWorkflowSchema: {
          version: 4,
          minimumRollbackCompatibleVersion: 4,
        },
      },
    },
    { requirePromoted: true },
