...
test('only promoted manifests can produce production Helm values', () => {
  ...
  assert.deepEqual(values.workflowSchema.compatibility, { minimumVersion: 4, maximumVersion: 4 });
...
test('forward rollback retains the migrated schema and restores only a compatible image set', () => {
  ...
  assert.deepEqual(values.workflowSchema.compatibility, { minimumVersion: 2, maximumVersion: 5 });
  assert.equal(values.release.production.database.managedWorkflowSchemaVersion, 5);
  ...
  const failedRelease = assertStudioServerReleaseManifest(
    {
      ...
      database: {
        managedWorkflowSchema: {
          version: 5,
          minimumRollbackCompatibleVersion: 5,
        },
      },
    },
    { requirePromoted: true },
