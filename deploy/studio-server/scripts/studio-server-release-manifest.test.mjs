...
test('only promoted manifests can produce production Helm values', () => {
  ...
  assert.deepEqual(values.workflowSchema.compatibility, { minimumVersion: 12, maximumVersion: 12 });
...
test('forward rollback retains the migrated schema and restores only a compatible image set', () => {
  ...
  assert.deepEqual(values.workflowSchema.compatibility, { minimumVersion: 2, maximumVersion: 12 });
  assert.equal(values.release.production.database.managedWorkflowSchemaVersion, 12);
