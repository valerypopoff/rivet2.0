...
class ManagedProviderGate {
  ...
  async createRegistrySecret() {
    const existing = await this.kubectl(['get', 'secret', this.config.registry.secretName, '-o', 'json'], {
      capture: true,
      allowFailure: true,
    });
    if (existing.exitCode !== 0 && !/NotFound|not found/iu.test(`${existing.stdout}${existing.stderr}`)) {
      throw new Error(
  ...
  async writeCandidateValues() {
    this.tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'rivet-managed-provider-gate-'));
    ...
    await fs.writeFile(this.candidateValuesFile, `${JSON.stringify(values, null, 2)}\n`, 'utf8');
  }

  async inspectReleaseHistory() {
    const result = await run(
      this.helmBin,
      ['history', this.config.release, '--namespace', this.config.namespace, '--output', 'json'],
    ...
    if (!result.stdout.trim()) {
      throw new Error(`[${runnerName}] Helm history for ${this.config.release} returned no data`);
    }
    let history;
    try {
      history = JSON.parse(result.stdout);
    } catch {
      throw new Error(`[${runnerName}] Helm history for ${this.config.release} did not return JSON`);
    }
    if (!Array.isArray(history) || history.length === 0) {
      throw new Error(`[${runnerName}] Helm history for ${this.config.release} did not contain a release revision`);
    }
    const deployed = history.filter((entry) => entry.status === 'deployed').at(-1);
    return {
      hasHistory: true,
      deployedRevision: Number.isInteger(deployed?.revision) ? deployed.revision : undefined,
    };
  }

  async upgradeCandidate({ reuseValues = false, setValues = {} } = {}) {
    if (!this.candidateValuesFile) throw new Error('Candidate values file was not initialized');
    const args = [
      'upgrade',
      '--install',
      this.config.release,
    ...
    args.push('--set-string', `gateway.mode=${this.config.gatewayMode}`);
    if (this.config.gatewayMode === 'external') {
      args.push(
        ...
        '--set',
        'metrics.proxyExporter.enabled=false',
      );
    }
    for (const [key, value] of Object.entries(setValues)) args.push('--set-string', `${key}=${value}`);
    ...
    if (this.config.gatewayMode === 'external') {
      ...
      if (/^kind:\s*Ingress\s*$/mu.test(manifest.stdout)) {
        throw new Error(`[${runnerName}] external gateway release unexpectedly contains an Ingress`);
      }
    }
  }
  ...
  async verifyGateway() {
    if (this.config.gatewayMode === 'external') {
      // The cluster owner may create an Ingress outside this Helm release or namespace.
      // Public HTTPS route probes below are the authority for this mode.
      return;
    }
    ...
    if (!hasHost) throw new Error(`[${runnerName}] no staging ingress serves ${expectedHost}`);
  }

  async request(pathValue, { method = 'GET', body, expectedStatus = 200, contains } = {}) {
    ...
    if (response.status !== expectedStatus) {
      throw new Error(`[${runnerName}] ${method} ${pathValue} returned ${response.status}, expected ${expectedStatus}`);
    }
    if (contains && !text.includes(contains)) {
      throw new Error(`[${runnerName}] ${method} ${pathValue} did not include the expected response marker`);
    }
    return { response, text };
  }

  async assertPublicHealth() {
    const { text } = await this.request('/readyz', { expectedStatus: 200 });
    if (this.config.gatewayMode === 'external') {
      let health;
      try {
        health = JSON.parse(text);
      } catch {
        throw new Error(`[${runnerName}] external gateway /readyz did not return API readiness JSON`);
      }
      if (health?.ok !== true || health?.state !== 'ready') {
        throw new Error(`[${runnerName}] external gateway /readyz did not report API readiness`);
      }
    }
  }
  ...
  async assertAppSettingsBaseline(baseline) {
    ...
    if (canonicalJson(current.text, 'Run recordings settings') !== baseline) {
      throw new Error(`[${runnerName}] persisted App Settings changed while rotating its encryption key`);
    }
  }
  ...
  async assertPublicSurface() {
    ...
    if (this.config.gatewayMode === 'external') {
      const endpointName = new URL(this.config.workflowProbe.path, this.config.baseUrl).pathname
        .split('/')
        .filter(Boolean)
        .at(-1);
      if (!endpointName) throw new Error(`[${runnerName}] workflowProbe.path must contain a published endpoint name`);
      await this.request(`/internal/workflows/${endpointName}`, {
  ...
  async validateInterruptionManifest(filePath) {
    ...
    const result = await this.kubectl(
      ['create', '--dry-run=client', '--namespace', this.config.namespace, '--output', 'json', '-f', filePath],
      { capture: true },
    );
    ...
    if (
      !resources.length ||
      resources.some(
        (resource) => resource.kind !== 'NetworkPolicy' || resource.metadata?.namespace !== this.config.namespace,
      )
    ) {
      throw new Error(
  ...
  async verifyDependencyInterruptions() {
    for (const interruption of this.config.interruptionManifests) {
      ...
      try {
        await this.kubectl(['apply', '--namespace', this.config.namespace, '-f', interruption.applyFile]);
        await waitFor(
          `${interruption.name} readiness failure`,
          async () => {
            const response = await fetch(new URL('/readyz', this.config.baseUrl), {
              redirect: 'manual',
              headers: this.config.requestHeaders,
              signal: AbortSignal.timeout(10_000),
            });
            if (response.ok) throw new Error('expected the staging runtime to become unready');
            return response;
          },
          90_000,
        );
      ...
      await waitFor(`${interruption.name} readiness recovery`, async () => this.assertPublicHealth(), 180_000);
    ...
    `${JSON.stringify(
      {
        ...
        images: Object.fromEntries(Object.entries(config.images).map(([name, image]) => [name, imageReference(image)])),
      },
      null,
      2,
    )}\n`,
