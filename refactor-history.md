...
137. **Separated graph dependency discovery from reachability traversal**
    ...
    - Actual line movement: The affected production sources moved by net `+51` physical lines: the 593-line dependency-discovery owner replaces 560 net lines in `graphReachability.ts`, while the shared core matcher/export add 18 lines. The planned deletion was not pursued because the ordered index and closed resolver set make graph-execution policy explicit rather than leaving it as a traversal switch.
...
138. **Centralized data-bus topology and presentation derivation**
    ...
    - Actual line movement: The affected production sources moved by net `+118` physical lines. `DataBusRail.tsx` shrank by 436 lines, while the reusable topology/presentation owner added 159 lines, the style owner 304, and the layout hook 94; the Canvas, NodePorts, and WireLayer integrations together removed three lines. The increase is intentional because topology, DOM observation, and CSS now have separate auditable owners.
...
140. **Completed the final five-phase refactor audit**


    - Why: The completed plan still had an outdated top-level status and the final file-tree guard exposed an import-boundary baseline overrun. The final repository verification also surfaced two newly published dependency advisories after the individual phases had passed.
    - How: Verified each phase against its live policy owner, public boundary, focused tests, and documentation. Recorded the missing Phase 3 and 4 physical line movement. Added the two-line `editorWorkflowState` editor seam so custom selectors share graph/project state without deep relative imports. Pinned the direct brace-expansion 5.x path to fixed 5.0.8, resolved PostCSS to fixed 8.5.22, and documented the only remaining 1.x/2.x no-fix brace-expansion toolchain paths with an expiry.
    - Result in numbers: The file-tree review queue fell from 158 to 152 long relative imports, below its 154-entry baseline. No phase runtime policy or serialized project behavior changed during this final audit.
    - Verification: The aggregate core/Node/app/app-executor/CLI suite, full production build, all-workspace lint, docs typecheck, style/freshness/architecture guards, file-tree guard, runtime-attribution and runtime-speed benchmarks, dependency audit, formatting checks, and `git diff --check` passed.

141. **Reduced custom browser-persistence code where a compatibility-safe library fit existed**


    - Why: `loc_reduce_with_libs.md` identified several custom implementations that looked replaceable by dependencies, but most candidates either changed a deliberately narrow contract or saved too little code to justify new compatibility risk. IndexedDB was the one behavior-preserving migration worth completing.
    - How: Removed the unused `useIndexedDb.ts` hook and migrated the three live app-owned IndexedDB stores to the direct `idb` dependency without changing database names, versions, object stores, key shapes, duplicate-`add` behavior, mutation order, transaction boundaries, or request-completion timing. Added one shared recoverable-connection owner so static data, Jotai persistence, and browser datasets reset cached handles after open failures, browser termination, or blocking schema upgrades. Static-data hook consumers now share a lazy connection instead of opening duplicate handles. The later hardening pass also ensured provider SSE timeouts and early consumers cancel both tee branches, while retaining fallback JSON only for responses that produce no recognized SSE event. The vendored FNV-1a implementation remains because its upstream package would add an ESM/CommonJS publishing exception; a zero-progress reusable UTF-8 buffer now throws rather than looping forever.
    - Deliberate non-migrations: Kept the existing SSE tokenizer rather than adopting `eventsource-parser`, because Rivet's legacy line-oriented event/data, multiline, bare-CR, and timeout behavior is a compatibility contract. Kept the vendored FNV-1a implementation and all other rejected candidates in `loc_reduce_with_libs.md` for their documented risk/benefit reasons.
    - Affected files/areas: app IndexedDB owners and their tests, `packages/app/src/utils/indexedDb.ts`, core `fetchEventSource.ts` and its characterization tests, core vendored `fnv1a.js` and golden tests, `developer-docs/APP-ARCHITECTURE.md`, `developer-docs/CORE-ENGINE.md`, `developer-docs/EXECUTION-DATA-FLOW.md`, and `loc_reduce_with_libs.md`.
    - Result in numbers: The initial migration deleted the 102-line unused hook and replaced the three live native IndexedDB wrappers with typed `idb` owners. The follow-up hardening centralized 31 lines of recoverable-connection policy and removed 22 duplicate lines from `BrowserDatasetProvider`, while adding explicit lifecycle coverage rather than relying on implicit browser behavior.
    - Verification: Legacy-schema, connection-invalidating, SSE timeout/early-exit, FNV buffer, and serialized-recording coverage passed in the 35-test focused suite. Focused app/core lint, core and app production builds, documentation/style validation, and `git diff --check` passed. The migration's earlier complete app/core suite runs also passed; the final hardening pass did not claim a combined full-suite result after its 120-second wrapper limit expired.

142. **Completed the LLM runtime ownership refactor**


    - Why: LLM Chat had accumulated overlapping owners for profile configuration, candidate resolution, response parsing, usage/diagnostics, tool rounds, connected Delegate lifetime, editor cache replay, and serialized-node migration. Those seams made fallback, cancellation, JSON validation, and tool continuation unnecessarily fragile to change.
    - How: Added a registry-backed profile ownership contract, provider-neutral invocation plans, resolved model candidates, a closed provider capability table, an append-only invocation journal, pure result projections, one response materializer, a narrow invocation coordinator, canonical tool codec/resolver/round helpers, a connected-continuation host, and explicit cache/migration boundaries. `LLMChatV2Node.process` now creates one journal, resolves runtime, delegates execution, applies the final projector, and returns outputs. `GraphProcessor` supplies connected-continuation scheduling callbacks without holding the invocation map.
    - Preserved behavior: Saved LLM Chat and LLM Profile data, public ports and DataValue shapes, provider request construction, retry-before-fallback ordering, fallback stickiness, direct tool return, internal and connected Delegate scheduling, replay/frozen restrictions, and legacy editor-cache compatibility remain unchanged. Confirmed defects were corrected: reserved tool arguments cannot overwrite Rivet inputs, every parallel Delegate invocation is checked for frozen/preloaded output, missing handler output is explicit, cancellation is not a fallback failure, final JSON validation inspects the actual Response value, cache hits never replay profile-attempt history, and transient provider-failure diagnostics are never cached as a later editor answer.
    - Guardrails and documentation: The `check-ai-runtime-boundaries` guard prevents `GraphProcessor` from importing profile/retry/response/cache projector policy. `developer-docs/LLM-CHAT-V2-CONTRACT.md` now names each runtime owner and its test surface, and `llm-refactor-plan.md` records the completed runtime scope versus the separately planned UX backlog.
    - Verification: Focused core lint plus the targeted journal, materializer, candidate, profile fallback, continuation, Delegate, connected-host, GraphProcessor, LLM Chat, and LLM Profile tests passed. The final post-refactor audit also passed all 1,156 complete-core tests with 8 intentional skips, the core ESM production build, the AI-runtime boundary check, and `git diff --check`.

## Residual Watchlist For Future Refactors

1. **GraphProcessor size and responsibility concentration**

   - Current state: Several targeted extractions landed: node-exclusion decisions live in `NodeExclusionPolicy.ts`, graph-boundary effects live in `GraphBoundaryEffects.ts`, and connected tool-call continuation now has dedicated planner/coordinator owners. `GraphProcessor.ts` is still a 3,784-line orchestration boundary with many execution policies.
