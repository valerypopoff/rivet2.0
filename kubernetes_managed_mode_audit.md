...
| 10. Hosted Evaluation continuation | P10-A/P10-B source implemented; live certification pending   | New managed hosted runs use durable server-owned claims, no-hidden-retry interruption semantics, an isolated worker tier, and atomic batch quotas; UI retry, retention, and live evidence remain open.              |
...
### Remaining Work (Operational Evidence and Lifecycle Policy)
...
3. **Retention:** define Evaluation-specific run/snapshot/recording limits, baseline/manual-pin exemptions, reference-safe deletion leases, and maintenance ownership before adding destructive Evaluation cleanup to Problem 9.
...
- Keep the snapshot and recording data out of project YAML. PostgreSQL owns run/scheduler metadata; existing Evaluation recording retention mechanisms continue to own replay artifacts.
...
- [ ] Evaluation retention protects active runs, baselines, manually retained artifacts, and shared dataset snapshots through a reviewed maintenance policy.
