...
The 2026-08-29 reassessment found six additional reliability boundaries. They concern release identity and rollback compatibility, cross-store disaster recovery, saturation control, durable maintenance/reconciliation, resumable hosted Evaluations, and production observability. Release/rollback and cross-store recovery are implemented in source and await operational certification. The first durable-maintenance slice is now implemented for managed workflow recording/project deletion; full reconciliation, the remaining domain adapters, Evaluation continuation, and production observability remain planned work. It also corrected two overstatements from the initial follow-up report: the singleton backend is already a documented residual boundary rather than a newly discovered defect, and fixed generic memory limits are not automatically safer than environment-specific limits established by load testing.
