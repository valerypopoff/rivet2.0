...
| 11. Production observability       | Partially implemented; metrics foundation complete                | Bounded API, reconciliation, and Evaluation-retention metrics plus opt-in Prometheus resources exist; SLOs, dashboards, tuned alerts, correlation, and several signal domains remain open.                                                                          |
...
- [ ] **Open implementation milestones:** Problem 8 still needs measured values before its non-CPU scaling and downstream-provider policies can be finalized. Problem 9 still lacks destructive policies for the remaining artifact domains plus outbox-age, scan-byte, and maintenance-freshness signals. Problem 10 still needs retention audit-to-enforce approval from retained evidence. Problem 11 still lacks settings-convergence, maintenance/outbox-freshness, proxy/executor, and correlation coverage plus versioned dashboards and tuned alerts.
...
5. Run statistics remain product-facing history. The metrics slice exposes hosted-Evaluation submission, queue, and worker signals plus recording drops, operational dependencies, reconciliation pages/generations/open findings, and Evaluation-retention candidates/deletions. It does not yet expose deletion-outbox age, maintenance lease/pass freshness, scan bytes, settings convergence, proxy/executor behavior, restart/OOM/eviction, or correlation/tracing; open-finding metrics also lack tuned incident routing and an authenticated detailed-finding workflow.
...
1. Extend the implemented low-cardinality, secret-free metrics surface beyond its HTTP/health/admission/hosted-Evaluation/recording/PostgreSQL/object-storage/runtime-library/reconciliation/Evaluation-retention baseline to settings convergence, maintenance lease/pass and deletion-outbox freshness, scan volume, proxy/executor behavior, and actionable integrity state.
...
- **Instrumentation:** retain the implemented API, reconciliation, and Evaluation-retention registry and extend it to settings convergence, maintenance lease/pass and deletion-outbox freshness, scan volume, proxy/executor behavior, and actionable integrity state. Do not turn API-only metrics into a public-proxy endpoint.
...
- [x] HTTP, readiness, published admission, hosted-Evaluation submission/queue/worker, recording persistence, PostgreSQL pool, managed object-storage, runtime-library, reconciliation, and Evaluation-retention signals have bounded labels and a pull-only scrape endpoint.
...
- [ ] Deletion-outbox backlog age, maintenance lease/pass freshness, settings convergence, proxy/executor behavior, and end-to-end correlation are observable without high-cardinality labels or sensitive payloads.
