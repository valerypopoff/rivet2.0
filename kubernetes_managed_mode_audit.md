...
| 8. Execution saturation            | Partially implemented; capacity-policy and public-admission slices complete | Public work is admitted per pod, all chart-managed writable volumes are bounded, and production must choose measured pod limits or explicit deferrals; measured capacity, Evaluation scheduling, and downstream load remain open. |
...
6. Add a production-like published-endpoint load gate using representative short, long, parallel, tool-using, and Code-node workflows. Add a concurrent Evaluation batch scenario after it is server-owned to prove batch work cannot starve public traffic. Derive requests, limits, HPA targets, and maximum replicas from that evidence while retaining the PostgreSQL connection formula.
...
**Implementation status (2026-08-29): implemented for managed endpoint-recording retention, explicit managed recording/project deletion, and known pre-commit upload failures. Schema migration 3 adds a PostgreSQL-fenced singleton maintenance worker and transactional deletion outbox. The control-plane API runs it at startup and on a five-minute timer by default; candidate selection stays in PostgreSQL and is bounded by the configured batch, while scalable execution replicas are explicitly disabled. Deleted recording, replay, revision, and project keys are queued in the metadata transaction; known keys uploaded before a later row/transaction failure are queued independently. Every queued key is rechecked for live database ownership before object deletion and retried with bounded exponential backoff. Full age-gated reconciliation for crash-window/prefix orphans, Evaluation retention, runtime-library scheduling, and operator metrics remain open.**
...
5. [`_env.tpl`](deploy/studio-server/helm/templates/_env.tpl), [`values.yaml`](deploy/studio-server/helm/values.yaml), and [`validate-values.yaml`](deploy/studio-server/helm/templates/validate-values.yaml) make the control-plane-only ownership and interval/lease/batch bounds chart-owned. Contract tests prove execution replicas do not schedule maintenance and callers cannot smuggle an override through `env`.

### Remaining Work

1. Known pre-commit upload failures now queue their exact object keys durably. The remaining gap is the crash window after object upload but before that queue write (plus unknown historical objects): no durable intent exists, so the safe next design remains a separately named stale-upload audit with a minimum age and database ownership recheck, not blind deletion.
2. [`runtime-libraries/managed/cleanup.ts`](packages/studio-server-api/src/runtime-libraries/managed/cleanup.ts) has a safe operator command, but no scheduled domain adapter under this maintenance owner.
3. Evaluation temporary recordings are still reclaimed opportunistically, and Evaluation run/snapshot/baseline retention needs reference-safe policy before it can share this worker.
4. There is no generic object-prefix scanner or scan checkpoint. Object-store listings are eventually consistent and cannot safely decide deletion alone; every future adapter needs a minimum age and database ownership recheck.
5. No maintenance metrics or operator UI yet expose last pass, lag, pending/blocked work, retry age, candidates, or missing-reference integrity failures.

### Guardrails and Rollout

- Migration 3 is additive. A normal v3 release verifies exactly schema `3`; only a guarded forward rollback renders the prior v2 image with `2..3` compatibility, preserving the migrated schema without asking serving pods to apply or reverse DDL.
...
- The only automatic adapter in this slice is bounded, unpinned endpoint-recording retention plus explicit recording/project deletion. Do not route Evaluation, runtime-library, or generic orphan deletion through it until each has its own policy and protected-artifact tests.
...
- [ ] A safe dry-run/reconcile pass inventories workflow, Evaluation, and runtime-library objects without deleting them.
- [ ] Missing referenced artifacts create integrity evidence/alerts, while true old orphans need two consistent audit passes before deletion.
- [ ] Evaluation, runtime-library, short-lived web-app transport, and stale-upload adapters have independent retention and protected-artifact semantics.
- [ ] Operations can inspect maintenance lag, pending/blocked count, retry age, scanned bytes, and last success.
...
Complete Problem 9 beyond its implemented workflow-recording/project slice: add dry-run reconciliation, stale-upload and runtime-library adapters, Evaluation retention, integrity evidence, and operator metrics. Reuse the shared lease/outbox primitives rather than creating a second scheduler.
