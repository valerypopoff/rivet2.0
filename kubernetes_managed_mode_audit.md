...
| 10. Hosted Evaluation continuation | P10-A source implemented; P10-B capacity isolation and live certification pending | New managed hosted runs use durable server-owned claims and no-hidden-retry interruption semantics; batch quota, public reservation, UI retry, retention, and live evidence remain open. |
...
- [x] **Implementation evidence:** Problems 1–7 have their scoped source/chart/test contracts; Problem 8 has its public-execution guardrails and provider-capacity harness; Problem 9 has its fenced retention/outbox and audit-only reconciliation inventory; and Problem 11 has its bounded metrics baseline. These claims are tied to the symbols, chart contracts, and focused tests cited in their sections.
- [ ] **Open source milestones:** Problem 8 still lacks its measured operating envelope and its Evaluation-dependent work-class isolation. Problem 9 still lacks destructive policies for the remaining artifact domains and operational freshness/outbox signals. Problem 10 has P10-A server-owned dispatch in source but still lacks P10-B batch isolation, retention policy, and an operator-visible retry affordance. Problem 11 still lacks the remaining signal domains, correlation, dashboards, and tuned alerts.
...
| Evaluation target/evaluator trials | Potentially bursty fan-out despite few authors | P10-A server-owned managed coordinator and downstream providers | P10-A removes browser ownership; P10-B must add a real batch quota and reserve published-execution capacity before the feature is enabled in the high-volume topology. |
...
7. Published workflows and web-app actions route to the execution Service, while UI/API and latest routes terminate on the backend. P10-A hosted Evaluation execution now uses durable server-owned claims on the execution API profile rather than the initiating editor. It intentionally has no separate batch quota yet, so the remaining P10-B work must reserve public capacity before the opt-in can be enabled in the high-volume topology. `/workflows-latest/...` remains a control/debug surface and must not be used as a product endpoint.
...
## Partially Implemented Problem 10: Hosted Evaluations Now Have a Durable Server-Owned Coordinator

**Implementation status (2026-08-30): P10-A is implemented in source and disabled by default. Managed Studio Server can accept an immutable hosted-Evaluation snapshot, persist its schedule in PostgreSQL, and execute it from the execution API profile rather than from the initiating browser. This is not yet the capacity-isolated batch service promised by the full problem: P10-B still needs a distinct batch quota/reserved public capacity, observable batch saturation, live disruption coverage, and a reviewed retention policy. No source/static result is treated as a production certificate.**
...
4. A worker moves a job from `claimed` to `accepted` immediately before `runEvaluationTrial`. Only a pre-accept claim can return automatically to `queued`, because no graph may have started: expiry recovers it, and a worker that notices shutdown after claiming it releases it immediately. An expired accepted lease or unexpected worker loss is persisted as `interrupted` with a terminal trial explanation; it is never hidden-retried. A privileged operator can call the authenticated explicit retry endpoint for selected interrupted job ids.
5. Coordinated claim, cancellation, retry, and deletion transitions lock jobs before the hosted parent and run projection; hosted deletion fences those same rows and deletes the projection in the one transaction. This prevents claim/cancel/retry/delete races from reviving or deleting a run after its terminal decision.
6. Cancellation blocks new claims, turns queued/claimed jobs into canceled evidence, aborts active local workers, and lets already accepted workers settle cancellation through their fence. Only a durable cancel request is represented as cancellation: heartbeat or fencing loss becomes `interrupted`. Cooperative API shutdown aborts active workers, but bounds its scheduler drain to one-third of the lease (1–10 seconds) so a non-cooperative graph cannot hold up API disposal; any uncooperative accepted job falls back to fenced lease recovery.
7. The execution adapter uses the same Node `GraphProcessor`, managed runtime libraries, project-reference loader, dataset provider, provider-attempt accounting, and recording store as hosted workflow execution. It preserves input values as raw portable JSON while preserving context values as real Rivet DataValues, avoiding ambiguous object decoding.
8. Hosted editor code probes a capability route and, when enabled, submits the immutable snapshot then observes the normal shared Evaluation history. Browser/local execution remains the fallback when hosted coordination is unavailable. An editor session cannot start a fallback run while it still owns a hosted run, because one Stop action must refer to one execution. Existing retained runs stay readable through the version normalizer and are not migrated in place.
9. Helm keeps the feature chart-owned and off by default. `hostedEvaluations` validates worker concurrency `1..8`, lease `15s..10m`, poll period `250ms..30s`, and at least one effective execution replica when the feature is enabled; control-profile pods never execute workers and execution-profile pods do. Environment overrides cannot bypass the chart values. Disabling the flag stops submission and claims but preserves scheduler rows for inspection/cancellation and blocks deletion of queued/running work, so a later re-enable cannot dispatch an orphan.

### Remaining Work (P10-B and Operational Evidence)

1. **Capacity isolation:** workers currently share the execution API tier with published traffic. Add a separate bounded Evaluation batch class/quota, reserve public admission capacity, and expose finite batch-versus-public saturation metrics before enabling this in a high-volume production topology.
2. **Disruption certificate:** add a live managed Kubernetes scenario that starts a multi-trial hosted Evaluation, closes the initiating browser, drains an execution worker after one acceptance, proves settled/queued/interrupted semantics, exercises cancellation, and checks no duplicate observations. Run it with immutable candidate images in Kind and protected provider staging.
3. **Operator affordance:** expose selected interrupted trials and the explicit retry command in the Evaluations Runs UI. Until then, retry remains an authenticated operator API action; a normal browser reload never retries accepted work.
4. **Retention:** define Evaluation-specific run/snapshot/recording limits, baseline/manual-pin exemptions, reference-safe deletion leases, and maintenance ownership before adding destructive Evaluation cleanup to Problem 9.
5. **Credentials and scope:** keep browser-only credentials out of the server snapshot. Any future credential projection must be an explicit allowed server-side contract, never an implicit browser-state copy.
6. **Feature disable:** disabling the chart flag stops submissions and claims, but it must leave existing scheduler rows inspectable/cancelable and block deletion of queued/running runs so a later enablement cannot dispatch orphaned work.

### Guardrails and Rollout

- Do not turn on `hostedEvaluations.enabled` merely to make a dashboard feature look available. Enable it only in managed storage and only after P10-B has reserved public endpoint capacity for the deployment topology.
- Never alter a retained client-owned run to become server-owned. Only new hosted submissions use the coordinator; legacy records remain normalizer-readable history.
- Do not add retry-on-error, retry-on-timeout, or retry-on-pod-loss policy around `runEvaluationTrial`. Graph and provider retry configuration remains authoritative; coordinator recovery is limited to never-accepted claims.
...
- [ ] P10-B proves that an Evaluation batch cannot consume reserved published-execution capacity or materially breach the public endpoint SLO.
- [ ] A live Kind and protected provider-staging disruption certificate proves browser loss, worker loss, cancellation, and no-duplicate observations.
...
P10-A now owns hosted dispatch durably. Complete P10-B with a separate bounded batch quota, reserved public-execution capacity, visible batch metrics, and live worker-loss/cancellation coverage. This order uses the new durable coordinator as the only scheduler/lease authority, so the batch quota can control real work without creating a second queue, lease system, or retention owner with different semantics.
...
| Durable Evaluation rows imply safe server continuation.                       | Partially supported.      | New managed hosted submissions use a durable PostgreSQL queue: never-accepted work can continue, while accepted worker loss becomes explicit interruption evidence with no hidden retry. Legacy/client-owned history is only normalized on read; P10-B capacity isolation and live disruption evidence remain open. |
...
| Three Evaluation authors imply negligible Evaluation execution load.           | Not supported.            | Authoring/control is low-volume, but one suite can fan out cases × trials × evaluators. P10-A now moves dispatch server-side; P10-B still needs a bounded batch quota that cannot starve public endpoints.                                                                                                                     |
