...
| 9. Retention and reconciliation    | Partially implemented; remaining deletion policies are gated            | A fenced owner, durable workflow-deletion retry, checkpointed audit inventory, bounded reconciliation telemetry, and the P10-C Evaluation lifecycle exist; no unknown object is automatically deleted.                                                                                                                           |
...
| 11. Production observability       | Partially implemented; control-plane freshness now instrumented         | Bounded API, reconciliation, Evaluation-retention, maintenance/outbox, and local settings-sync metrics plus opt-in Prometheus resources exist; SLOs, dashboards, tuned alerts, correlation, and proxy/executor signals remain open.                                                                                              |

### Evidence Ledger (2026-08-30)

- [x] **Implementation evidence:** Problems 1–7 have their scoped source/chart/test contracts; Problem 8 has its public-execution guardrails, provider-capacity harness, and source-level Evaluation work-class isolation; Problem 9 has its fenced retention/outbox and audit-only reconciliation inventory; Problem 10 has durable server-owned dispatch, a dedicated quota-bounded worker tier, P10-C audit-first replay/snapshot lifecycle, and P10-D's guarded retry UI; and Problem 11 has its bounded metrics baseline. These claims are tied to the symbols, chart contracts, and focused tests cited in their sections.
- [ ] **Open implementation milestones:** Problem 8 still needs measured values before its non-CPU scaling and downstream-provider policies can be finalized. Problem 9 still lacks destructive policies for the remaining artifact domains plus scan-byte accounting. Problem 10 still needs retention audit-to-enforce approval from retained evidence. Problem 11 still lacks cluster-wide settings-convergence, proxy/executor, and correlation coverage plus versioned dashboards and tuned alerts.
...
4. The status command and finite Prometheus metrics expose reconciliation phase, completion, page outcomes/returned-item counts, open counts, durable deletion-outbox pending/claimed/blocked age, and control-plane maintenance freshness. They do not expose scan byte volume, SLO alerts, dashboards, or an authenticated detailed-finding workflow.
...
- [ ] Operations still need scan-byte accounting plus staged, owned dashboards/SLOs for the implemented outbox/maintenance signals; the status command remains the safe aggregate reconciliation baseline.
...
1. Validate the implemented low-cardinality, secret-free maintenance/outbox, reconciliation, and local settings-sync signals under staged load. Add only the remaining bounded domains: byte-level scan accounting if it proves operationally useful, proxy/executor behavior, and actionable integrity state. Do not misrepresent a local synchronization success as cluster-wide convergence.
...
- **Instrumentation:** retain the implemented API, reconciliation, Evaluation-retention, maintenance/outbox, and local settings-sync registry; validate its overhead and cardinality in staging, then add only justified scan-byte, proxy/executor, and actionable-integrity signals. Do not turn API-only metrics into a public-proxy endpoint.
...
- [x] HTTP, readiness, published admission, hosted-Evaluation submission/queue/worker, recording persistence, PostgreSQL pool, managed object-storage, runtime-library, reconciliation, Evaluation-retention, maintenance/outbox freshness, and local App Settings synchronization signals have bounded labels and a pull-only scrape endpoint.
...
- [ ] Proxy/executor behavior, end-to-end correlation, and any needed cluster-wide settings-convergence evidence are observable without high-cardinality labels or sensitive payloads.
