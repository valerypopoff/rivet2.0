...
| 10. Hosted Evaluation continuation | P10-A/P10-B/P10-C source implemented; audit-first rollout pending | New managed hosted runs use durable server-owned claims, no-hidden-retry interruption semantics, an isolated worker tier, atomic batch quotas, and a fenced replay/snapshot lifecycle; UI retry, audit-to-enforce retention rollout, and live evidence remain open. |
...
- [x] **Implementation evidence:** Problems 1–7 have their scoped source/chart/test contracts; Problem 8 has its public-execution guardrails, provider-capacity harness, and source-level Evaluation work-class isolation; Problem 9 has its fenced retention/outbox and audit-only reconciliation inventory; Problem 10 has durable server-owned dispatch, a dedicated quota-bounded worker tier, and P10-C audit-first replay/snapshot lifecycle; and Problem 11 has its bounded metrics baseline. These claims are tied to the symbols, chart contracts, and focused tests cited in their sections.
- [ ] **Open source milestones:** Problem 8 still lacks its measured operating envelope, non-CPU scaling policy, and downstream-provider limits. Problem 9 still lacks destructive policies for the remaining artifact domains and operational freshness/outbox signals. Problem 10 still lacks retained audit-to-enforce rollout evidence and an operator-visible retry affordance. Problem 11 still lacks the remaining signal domains, correlation, dashboards, and tuned alerts.
- [ ] **No retained remote certificate is claimed by this audit:** there is no recorded successful remote full-Kind run, provider-staging release/capacity run, provider restore drill, or prior-image compatibility run in the evidence cited here. A bootstrap skip is not certification.
...
2. **Operator affordance:** expose selected interrupted trials and the explicit retry command in the Evaluations Runs UI. Until then, retry remains an authenticated operator API action; a normal browser reload never retries accepted work.
3. **Retention rollout:** inspect `rivet_managed_evaluation_retention_candidates` in `audit` mode under representative load, retain that evidence, then approve `evaluationRetention.mode: enforce`. Revisit only with an explicit run-history retention policy; P10-C deliberately keeps compact run results and baseline/failure/manual recordings.
4. **Credentials and scope:** keep browser-only credentials out of the server snapshot. Any future credential projection must be an explicit allowed server-side contract, never an implicit browser-state copy.
5. **Operating envelope:** select the Evaluation replica count, concurrency, quotas, resource pairs, placement/priority, and provider budget from retained staging evidence. Static bounds prevent unbounded work; they do not establish safe production values.
...
5. Run statistics remain product-facing history. The metrics slice exposes hosted-Evaluation submission, queue, and worker signals plus recording drops and operational dependencies, but does not yet cover maintenance/outbox lag, settings convergence, proxy/executor behavior, missing-reference integrity, restart/OOM/eviction, or correlation/tracing.
...
1. Extend the implemented low-cardinality, secret-free metrics surface beyond its HTTP/health/admission/hosted-Evaluation/recording/PostgreSQL/object-storage/runtime-library baseline to settings convergence, maintenance/reconciliation, proxy/executor behavior, and integrity state.
...
- **Instrumentation:** retain the implemented API registry and extend it to settings convergence, maintenance, proxy/executor behavior, and integrity state. Do not turn API-only metrics into a public-proxy endpoint.
...
- [x] HTTP, readiness, published admission, hosted-Evaluation submission/queue/worker, recording persistence, PostgreSQL pool, managed object-storage, and runtime-library signals have bounded labels and a pull-only scrape endpoint.
...
P10-A/P10-B now own hosted dispatch, a separate bounded batch quota, reserved public-execution capacity, and visible batch metrics. Certify them with live worker-loss/cancellation and concurrent batch-versus-public coverage. The durable coordinator remains the only scheduler/lease authority, so the capacity contract does not create a second queue, lease system, or retention owner with different semantics.
