...
function currentMigration(): AppliedMigration {
  const migration = MANAGED_WORKFLOW_SCHEMA_MIGRATIONS[0];
  assert.ok(migration);
  return {
...
function createCurrentDatabase(): FakeSchemaDatabase {
  ...
  return createDatabase({
    ...
    schemaReady: true,
    migrations: new Map([[migration.version, migration]]),
  });
}
...
test('managed schema migration definitions and compatibility probes remain coherent', () => {
  assert.equal(CURRENT_MANAGED_WORKFLOW_SCHEMA_VERSION, 1);
  assert.deepEqual(MANAGED_WORKFLOW_SCHEMA_MIGRATIONS.map(({ version }) => version), [1]);
  assert.equal(
  ...
  assert.equal(MANAGED_WORKFLOW_SCHEMA_REQUIRED_COLUMNS.length, 125);
  ...
  assert.equal(MANAGED_WORKFLOW_SCHEMA_REQUIRED_COLUMN_DEFAULTS.length, 34);
  ...
  assert.equal(MANAGED_WORKFLOW_SCHEMA_REQUIRED_CONSTRAINTS.length, 35);
...
test('concurrent schema migrators serialize and apply the baseline once', async () => {
  ...
  assert.equal(database.applyCount, 1);
  assert.deepEqual([...database.migrations.keys()], [1]);
  assert.ok(results.every(({ currentVersion }) => currentVersion === 1));
  assert.equal(results.filter(({ appliedVersions }) => appliedVersions.length === 0).length, 3);
  assert.equal(
    results.filter(({ appliedVersions }) => appliedVersions.length === 1 && appliedVersions[0] === 1).length,
    1,
  );
  assert.equal(
...
test('migration baselines an existing unversioned schema without losing compatibility', async () => {
  ...
  assert.deepEqual(result, { currentVersion: 1, appliedVersions: [1] });
  assert.equal(database.migrationTableExists, true);
  assert.equal(database.schemaReady, true);
  assert.equal(database.migrations.get(1)?.checksum, currentMigration().checksum);
});
...
test('verify mode takes a shared lock and never applies schema SQL', async () => {
  ...
  assert.deepEqual(result, { currentVersion: 1, appliedVersions: [] });
...
test('schema compatibility rejects future and modified migration histories', async (t) => {
  await t.test('future version', async () => {
    const database = createCurrentDatabase();
    database.migrations.set(2, { version: 2, name: 'future', checksum: 'f'.repeat(64) });
    await assert.rejects(
      verifyManagedWorkflowSchema(createPool(database), { logger: quietLogger }),
...
test('schema compatibility rejects a ledger marked current when required objects are missing', async (t) => {
  ...
  await t.test('index table', async () => {
    ...
    await assert.rejects(
      verifyManagedWorkflowSchema(createPool(database), { logger: quietLogger }),
      /"tableName":"workflows".*"tableName":"wrong_table"/,
    );
  });
  ...
  await t.test('index definition', async () => {
    ...
    await assert.rejects(
      verifyManagedWorkflowSchema(createPool(database), { logger: quietLogger }),
      /"keyExpressions":\["folder_relative_path"\].*"keyExpressions":\["wrong_column"\]/,
    );
  });
  ...
  await t.test('constraint definition', async () => {
    const database = createCurrentDatabase();
    database.invalidConstraint = MANAGED_WORKFLOW_SCHEMA_REQUIRED_CONSTRAINTS[0]?.join('/') ?? null;
    await assert.rejects(
      verifyManagedWorkflowSchema(createPool(database), { logger: quietLogger }),
