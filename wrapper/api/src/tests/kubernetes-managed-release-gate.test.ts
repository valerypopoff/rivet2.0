...
import { buildManagedProviderGateConfig } from "../../../../scripts/lib/kubernetes-managed-provider-gate-config.mjs";
...
test("managed release gate reserves enough workers for deterministic execution node-drain coverage", async () => {
  const overlay = await fs.readFile(
    path.join(rootDir, "charts", "overlays", "managed-release-gate.yaml"),
    "utf8",
  );
  const kindTopology = await fs.readFile(
    path.join(rootDir, "scripts", "kind-managed-release-cluster.yaml"),
    "utf8",
  );
...
test("managed release disruption gate covers WebSocket owner loss and managed dependency recovery", async () => {
  const [runner, fixture, overlay] = await Promise.all([
    fs.readFile(
      path.join(rootDir, "scripts", "kubernetes-managed-release-gate.mjs"),
      "utf8",
    ),
    fs.readFile(
      path.join(
  ...
  assert.match(runner, /resumed\.terminal,\s+120_000/);
...
test("provider staging gate preserves secret inputs outside uploaded artifacts", async () => {
  const [runner, workflow] = await Promise.all([
    fs.readFile(
      path.join(rootDir, "scripts", "kubernetes-managed-provider-gate.mjs"),
      "utf8",
    ),
    fs.readFile(
      path.join(rootDir, ".github", "workflows", "build-images.yml"),
      "utf8",
    ),
  ]);
