...
For local OAuth testing, set the auth mode to OAuth and choose `Local dummy` as the provider in `Settings` -> `Web apps` -> `Auth`. The wrapper then sends the Sign in flow to the active published-app auth route, usually `/apps/auth/dummy`, where the tester can enter any email. Submitting the form still returns through the same callback/session-cookie path that real OAuth creates, so allowed-email lists are tested without a real provider. Dummy OAuth is localhost-only by default and should not be used for production deployments.
