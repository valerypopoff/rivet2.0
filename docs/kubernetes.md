...
Published workflow endpoints in the split `execution` deployment also need this settings volume to be visible if they should consume UI-managed recording limits or proxy values. Use a shared RWX `storage.appData.existingClaimName` for execution pods; recording queue/retention limits and runtime endpoint proxy values are not read from `.env`, Vault dotenv, or deployment `RIVET_RECORDINGS_MAX_*` / `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY` variables.
