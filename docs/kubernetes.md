...
`develop-rivet2` uses the separate `Verify Develop` workflow. `npm run test` initializes the same upstream Rivet source through its pretest hook, then runs the full repository checks without building images, pushing to GHCR, or creating a Kind cluster. The normal image workflow is reserved for pushes to `main-rivet2`: it runs the managed-mode smoke suite through `npm run verify:kubernetes:managed-live`; scheduled runs and the `run_managed_kubernetes_disruption` workflow-dispatch input run the broader `npm run verify:kubernetes:managed-disruption` suite. The latter adds the controlled proxy rollout, execution-node drain, deliberately long web-app WebSocket forced-owner-loss/reconnect check, App Settings encryption-key rotation, and PostgreSQL/MinIO outage/recovery checks. GitHub executes scheduled workflows only from the repository default branch, which is `main-rivet2`. Each image-gate run uploads non-secret manifests, Kubernetes events, pod descriptions, and logs. Generated credentials are supplied only to Kubernetes and are never placed in artifacts.
