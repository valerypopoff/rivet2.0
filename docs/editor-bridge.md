...
7. Hosted editor project tabs show only the project title. The wrapper strips upstream's bracketed file-name suffix from `ProjectSelector` at build time because the workflow tree already owns the file/path context and this repo does not commit changes inside the vendored `rivet/` tree. That transform is scoped to the upstream file and tolerant of LF/CRLF line endings from linked Rivet checkouts. It recognizes the older all-tabs suffix expression, the active-tab-only suffix expression, and the preview-tab-aware active expression while preserving upstream's preview tab state, but it still fails loudly if the upstream tab-label expression changes again and needs a fresh review.
...
The reliability API owns the clock and rejects caller timestamp fields. Every hosted runtime request must carry a project id. The outer Project Settings modal loads the list only while its **LLM profile suspension** tab is open, refreshes every five seconds, and can list and clear only the active project's entries. It shows active suspensions, profiles awaiting their one recovery attempt, and recovery attempts currently in progress, so retained recovery state is never hidden behind an empty panel. Even a single-profile clear carries that project id, and the HTTP surface exposes no unscoped list or clear operation. The runtime store exposes atomic begin/finish/renew operations; it is not a browser-local cache and does not fall back to local state when the server call fails.
