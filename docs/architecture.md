...
- `Settings` in the workflow-library bottom action stack opens the wrapper-owned app settings modal. The modal uses the same tab chrome as Project Settings. The `General` tab surfaces app identity, route prefixes, and web-app auth mode from the runtime dashboard config. The `Node executor proxy` tab stores runtime `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` values in app data so both hosted editor Node execution and API/headless endpoint execution can use an outbound proxy without reading those values from `.env`.
...
- `/api/app-settings/*` manages wrapper-owned app settings such as the persisted Node executor outbound proxy values.
