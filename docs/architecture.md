...
- `Settings` in the workflow-library bottom action stack opens the wrapper-owned app settings modal. The modal uses the same tab chrome as Project Settings. App Settings tab panels do not repeat the active tab name as a heading, and save/error feedback appears in the action row beside the buttons. The `General` tab surfaces app identity and route prefixes from the runtime dashboard config. The `Web app auth` tab stores the browser auth mode and OAuth provider details in app data instead of reading the old web-app auth env vars. The `Run recordings` tab stores recording queue depth, per-endpoint run history length, and age retention in app data instead of reading the old recordings limit env vars. The `Node executor proxy` tab stores runtime `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` values in app data so both hosted editor Node execution and API/headless endpoint execution can use an outbound proxy without reading those values from `.env`.
...
- Web-app auth and OAuth provider settings are not read from `.env`. Configure them in `Settings` -> `Web app auth`; the API stores them under `RIVET_APP_DATA_ROOT` and applies them to `${RIVET_PUBLISHED_APPS_BASE_PATH}` and `${RIVET_LATEST_APPS_BASE_PATH}`. The supported modes are `Rivet key`, `OAuth`, and `No app gate`. OAuth web apps are fail-closed when their allowed-email list is empty. Legacy `RIVET_WEB_APPS_AUTH_MODE` and `OAUTH_*` env values are ignored.
