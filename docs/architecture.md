...
- `Settings` in the workflow-library bottom action stack opens the wrapper-owned app settings modal. The modal uses the same tab chrome as Project Settings. App Settings tab panels do not repeat the active tab name as a heading, and save/error feedback appears in the action row beside the buttons. The `General` tab surfaces app identity and currently active route prefixes from the runtime dashboard config. The `Storage` tab stores the workflow/runtime-library storage mode, filesystem artifacts host path, managed database settings, and managed object-storage credentials in app data; saved values win over deployment env defaults after the API/executor processes restart or Kubernetes pods roll out, and secrets are not returned to the browser. The `Workflow endpoints` tab owns published/latest workflow endpoint route slugs. The `Web apps` tab owns web-app route slugs and browser auth: its `Routes` section stores the published/latest app top-level URL slugs in app data, and its `Auth` section stores the browser auth mode and OAuth provider details in app data instead of reading the old web-app auth env vars. The workflow and web-app route tabs write the shared `settings/public-routes.json` contract and wait for `/api/config` to reflect changes before showing success. The `Run recordings` tab stores recording queue depth, per-endpoint run history length, and age retention in app data instead of reading the old recordings limit env vars. The `Node executor proxy` tab stores runtime `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` values in app data so both hosted editor Node execution and API/headless endpoint execution can use an outbound proxy without reading those values from `.env`. The `Docker` tab is explicitly launcher-only and has no Kubernetes runtime effect.
...
- `/api/app-settings/*` manages wrapper-owned app settings such as run-recording retention limits, persisted Node executor outbound proxy values, web-app route slugs, and web-app browser auth/OAuth settings.
...
- `RIVET_REQUIRE_UI_GATE_KEY` enables the browser-side nginx gate for the main dashboard/editor UI.
...
- Web-app auth and OAuth provider settings are not read from `.env`. Configure them in `Settings` -> `Web apps` -> `Auth`; the API stores them under `RIVET_APP_DATA_ROOT` and applies them to the active published/latest web-app route families. The supported modes are `Rivet key`, `OAuth`, and `No app gate`. OAuth web apps are fail-closed when their allowed-email list is empty. Legacy `RIVET_WEB_APPS_AUTH_MODE` and `OAUTH_*` env values are ignored.
- `RIVET_UI_TOKEN_FREE_HOSTS` lists hosts that bypass the UI gate for the main UI, bypass web-app auth in any web-app auth mode, and also bypass public workflow bearer auth.
...
- `RIVET_ARTIFACTS_HOST_PATH` remains a launcher-level bootstrap/default for filesystem host bind mounts. `Settings` -> `Storage` records the preferred artifacts host path, but Docker/Compose/Kubernetes still need their mounts to point at the desired host storage before the container starts.
