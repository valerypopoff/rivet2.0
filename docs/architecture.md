...
- The hosted `state/settings` override mirrors upstream settings exports while keeping hosted executor/debugger defaults and the wrapper update-check modal atom. When upstream adds browser UI settings, the wrapper override must expose those same atoms/helpers so the hosted production bundle sees the same public module shape.
