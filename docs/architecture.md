...
- `Settings` in the workflow-library bottom action stack opens the wrapper-owned app settings modal. The modal uses the same tab chrome as Project Settings. App Settings tab panels do not repeat the active tab name as a heading, and save/error feedback appears in the action row beside the buttons. The `General` tab surfaces app identity, route prefixes, and web-app auth mode from the runtime dashboard config. The `Run recordings` tab stores recording queue depth, per-endpoint run history length, and age retention in app data instead of reading the old recordings limit env vars. The `Node executor proxy` tab stores runtime `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` values in app data so both hosted editor Node execution and API/headless endpoint execution can use an outbound proxy without reading those values from `.env`.
...
- `/api/app-settings/*` manages wrapper-owned app settings such as run-recording retention limits and persisted Node executor outbound proxy values.
...
- `RIVET_WEB_APPS_AUTH_MODE` selects the browser auth model for `${RIVET_PUBLISHED_APPS_BASE_PATH}` and `${RIVET_LATEST_APPS_BASE_PATH}`: `ui-gate` reuses the main UI key prompt, `oauth` redirects app visitors through a configured OAuth provider and then applies each app's explicit allowed-email list, and `none` leaves app routes open at the API layer. OAuth web apps are fail-closed when their allowed-email list is empty.
...
- `OAUTH_AUTHORIZE_URL`, `OAUTH_TOKEN_URL`, `OAUTH_USER_URL`, `OAUTH_CLIENT_ID`, `OAUTH_CLIENT_SECRET`, and `OAUTH_SESSION_SECRET` configure generic OAuth mode for web apps. Optional `OAUTH_CALLBACK_URL`, `OAUTH_SCOPES`, `OAUTH_EMAIL_CLAIM`, `OAUTH_SESSION_TTL_SECONDS`, and `OAUTH_CLIENT_AUTH_METHOD` tune provider details. External OAuth provider URLs must use `https`; `http` is allowed only for localhost development endpoints. `OAUTH_CLIENT_AUTH_METHOD` defaults to body credentials and can be set to `basic` for providers that require HTTP Basic client authentication at the token endpoint. `OAUTH_PROVIDER=dummy` is a local-only test provider that renders `/apps/auth/dummy`, accepts a test email, and then uses the same callback/session path as real OAuth. `OAUTH_DEBUG_LOG_PROFILE=true` is a temporary troubleshooting flag that logs the raw provider profile JSON so operators can discover the right `OAUTH_EMAIL_CLAIM`.
