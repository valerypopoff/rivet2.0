...
- `Object storage`
  ...
  - execution-plane `RIVET_APP_DATA_ROOT` may remain ephemeral because the current published execution path does not use it as authoritative workflow state; if UI-managed Node executor proxy settings should affect split execution pods, they must receive the shared `settings/node-executor-proxy.json` app-data file because runtime endpoint proxy values are not read from `.env` or deployment proxy environment variables
