...
- dashboard controllers belong in `wrapper/web/dashboard/`
  ...
  - keep bottom panel actions in the mounted workflow-library panel; the app-level `Settings` modal uses the shared settings-tab chrome, shows general runtime config, and owns the Node executor proxy form. The `About` modal shows the official `Rivet Studio Server` app name and reads the hosted app version from the root `package.json` through the Vite build constant
