...
- `npm run setup`
  ...
  - wrapper-owned snapshots use `YARN_NODE_LINKER=node-modules` consistently for both installation and package builds, clear inherited `NODE_OPTIONS` PnP preloads, and remove stale Yarn Plug'n'Play loader artifacts both before and after installation; a linked external checkout is never modified and retains its own PnP or node-modules setup
