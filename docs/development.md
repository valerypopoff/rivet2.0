...
- Project Settings reads route prefixes from the runtime `/api/config` response, not just from the prebuilt web bundle. After changing route-prefix values such as `RIVET_PUBLISHED_APPS_BASE_PATH` or `RIVET_LATEST_APPS_BASE_PATH`, `npm run prod:restart` plus a browser reload is enough for nginx, API routing, and the displayed Project Settings controls to agree. Web-app auth/OAuth settings are no longer route env; change them in App Settings -> `Web app auth`.
