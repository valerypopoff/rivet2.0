...
- `npm run setup`
  ...
  - wrapper-owned snapshots persist `nodeLinker: node-modules` in their local `.yarnrc.yml`, set the matching child-process environment, remove inherited `NODE_OPTIONS` PnP preloads while preserving unrelated Node options, and remove stale Yarn Plug'n'Play loader files both before and after installation; a linked external checkout is never modified and retains its own PnP or node-modules setup
