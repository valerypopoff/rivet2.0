...
| `npm run dev:docker:config` | Renders the unexpanded Docker dev Compose topology without starting containers or printing dotenv values | Inspect launcher/Compose structure safely |
...
- Changing a launcher dotenv credential does not mutate an already-running process environment. Recreate the relevant Docker services with `npm run dev:docker:recreate` for development or `npm run prod:restart` for production; a browser reload alone is not enough. Browser-executor aliases additionally require the exact variable name in `RIVET_ENV_ALLOWLIST`, then a container recreate and browser reload, unless the credential is supplied through Rivet Settings or the API-key input port. Protected server-credential names remain denied even when listed; this includes sensitive-name variants and common database credentials such as `PGPASSWORD`, `MONGODB_URI`, and `REDIS_URL`. Use a purpose-specific `*_API_KEY` alias for an intentionally browser-visible provider key instead of reusing a password, secret, token, credential, database, signing, encryption, or object-storage credential name.
- `Settings` -> `Environment variables` stores runtime overrides in `settings/environment-variables.json` under app data. The compact settings table keeps already-saved values behind a fixed bullet mask by default, while unsaved edits remain visible as you type; an authenticated per-variable eye action retrieves only the selected saved value with a non-cacheable response, and hiding it removes that revealed value from the UI state. These values override the same names from the launcher dotenv for every **new** workflow endpoint, web-app, and editor Node-executor run; active runs retain their captured values. Provider credential resolution and `process.env` in Node-executed Code nodes use that same immutable per-run overlay, so concurrent runs cannot see each other's values. The API process reads the overlay at run start and the app executor retrieves it through an executor-authenticated internal route, so saving does not need a container restart. Kubernetes backend and execution workloads share the app-data PVC; the co-located editor executor uses loopback to retrieve the overlay. The `Browser` checkbox opts that individual value into Browser-executor lookup and refreshes open hosted editor tabs after a save. A browser-enabled value is intentionally visible to browser JavaScript, while sensitive names such as Rivet keys, tokens, passwords, database settings, and OAuth secrets are rejected from browser exposure even if selected. External Remote Debugger processes remain independently owned and do not receive wrapper-managed variables.
- App Settings -> `General` controls trusted-host bypasses. `Trusted hosts` writes exact hostnames/IPs to `settings/trusted-hosts.json`; when the proxy sees one of those browser-facing hosts, it bypasses the UI key gate, web-app auth, and workflow endpoint bearer checks. App Settings -> `Shell execution` controls the editor's allowed server-command limits: `Command timeout` is saved in seconds and `Maximum captured output` is saved in MiB for the UI while the API stores bytes internally. These limits apply to editor-side operations such as Git history and revision comparison, not workflow or endpoint execution. The legacy `RIVET_UI_TOKEN_FREE_HOSTS` env var is ignored/rejected, and the proxy hot-reloads the saved trusted-host include from the shared app-data volume.
...
- Managed workflow schema changes live in ordered immutable migrations under `wrapper/api/src/routes/workflows/managed/schema-migrations.ts`. Migration 1 is the baseline SQL exported from `schema.ts`; never append a released change to that string because its committed checksum literal and deployed ledgers both verify it. Add migration N+1 with a new checksum literal, keep it transactional and expand-and-contract compatible, extend the complete column/default, table-privilege, operational-index, semantic-constraint, and required-function manifests, and add a prior-version plus concurrent-contender test. Table probes must continue to reject row-level security and missing API-role DML privileges; index probes must validate key expressions, ordering, predicates, and `indisvalid`/`indisready`; constraint probes must reject `NOT VALID` state; function probes must validate behavior-bearing catalog metadata and the stored body checksum rather than name alone. `npm run workflow-schema:migrate` applies schema migrations and `npm run workflow-schema:verify` performs a read-only compatibility check. These are database-schema commands, not aliases for `workflow-storage:migrate` / `workflow-storage:verify`, which move workflow data. Docker API startup defaults to migration mode. The Helm chart runs the migration command once in a pre-install/pre-upgrade Job using an isolated ephemeral app-data root, then forces serving API pods into verify-only mode.
...
- the API image uses upstream Rivet's wrapper-facing `yarn build:runtime` and `yarn build:hosted-web-deps` scripts to build `core + node + evaluations`, then links `wrapper/api` to those built package directories before compiling the API; this keeps hosted endpoint execution and durable Evaluations storage on the same Rivet source tree as the editor and executor
- the web image uses upstream Rivet's `yarn build:hosted-web-deps` script to build `core + evaluations` without the Dockerfile knowing Rivet's internal workspace build order
...
- the Docker dev API waits for the web service to populate the shared `rivet_node_modules` volume, then copies `rivet/packages/core`, `rivet/packages/node`, and `rivet/packages/evaluations` into container-local `/tmp/rivet-source`, attaches `/workspace/rivet/node_modules` beside that copy, and points the generated `@valerypopoff/rivet2-core`, `@valerypopoff/rivet2-node`, `@valerypopoff/rivet2-evaluations`, and `@rivet2/*` package overlays at the internal copy. Keeping this staging copy outside the `/app` bind mount avoids Windows host-volume link/copy failures while preserving Node package resolution inside the container, avoiding a duplicate upstream dependency install, and avoiding API helper links in the external Rivet checkout.
...
- API `tsc` builds include `src/tests`, so both API Dockerfiles explicitly stage any script module, plus its adjacent declaration file when present, that a test imports statically. Keep that list in sync when adding a static `../../../../scripts/...` test import; dynamic test-only imports do not need an image-time module-resolution dependency.
...
- the hosted web image builds upstream `rivet/packages/app` through `wrapper/web/vite.config.ts`, not through upstream Rivet's app Vite config. When upstream app/core code imports browser-only virtual modules or browser runtime dependencies such as `nspell`, `dictionary-en`, `rivet-cspell-words`, or Zod's V4 API surface, mirror the required Vite plugin/dependency seam in the wrapper config and cover it in `wrapper/web/tests/vite-aliases.test.ts`. The hosted bundle explicitly resolves bare `zod` imports to `zod/v4`, so upstream core schemas do not accidentally receive Zod's legacy default surface.
