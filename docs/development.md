...
- `npm run setup`
  ...
  - removes stale Yarn Plug'n'Play loader artifacts from a wrapper-owned `rivet/` snapshot before that enforced node-modules install; a linked external checkout is never modified and retains its own PnP or node-modules setup
