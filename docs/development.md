...
- App Settings saves use unique temporary files followed by rename for `settings/node-executor-proxy.json`, `settings/executor-url-overrides.json`, `settings/run-recordings.json`, `settings/runtime-limits.json`, `settings/deployment-storage.json`, `settings/workflow-endpoint-auth.json`, and `settings/web-app-auth.json`, so overlapping saves do not share the same temporary path.
