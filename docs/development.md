...
- Project Settings reads route prefixes and web-app auth mode from the runtime `/api/config` response, not just from the prebuilt web bundle. After changing values such as `RIVET_PUBLISHED_APPS_BASE_PATH`, `RIVET_LATEST_APPS_BASE_PATH`, or `RIVET_WEB_APPS_AUTH_MODE`, `npm run prod:restart` plus a browser reload is enough for nginx, API routing, and the displayed Project Settings controls to agree.
