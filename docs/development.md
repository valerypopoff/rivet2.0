...
- `npm run setup`
  ...
  - setup strips only inherited NODE_OPTIONS PnP preloads from every bootstrap child process while preserving unrelated Node options, and removes stale embedded PnP loader files before and after installation; a linked external Rivet checkout is never modified and retains its own PnP or node-modules setup
  - wrapper setup verifies the physical `rivet/node_modules` layout after an embedded install and fails before a package build if that layout is absent, instead of accidentally compiling against a stale PnP loader
  - local host-based API work and both GitHub workflows use Node 24+, matching the API image and its `node:sqlite` requirement
  - applies that same linker policy to the local Node-executor watch command, so `npm run dev:local` and `npm run dev:local:executor` work with either an embedded snapshot or an external PnP checkout; the direct executor command first runs the standard dependency bootstrap
...
- API `tsc` builds include `src/tests`, so both API Dockerfiles explicitly stage any script module, plus its adjacent declaration file when present, that a test imports statically. The managed Kubernetes release/provider gate config helpers are an example. Keep that list in sync when adding a static `../../../../scripts/...` test import; dynamic test-only imports do not need an image-time module-resolution dependency.
...
- Wrapper regressions built from upstream Rivet fixtures must derive fixture project and graph IDs from the parsed fixture instead of copying generated IDs into the test. For filesystem project-reference moves, cover a reused former hint path as well as the moved target: the wrapper must verify a hinted project's immutable ID before accepting it, then resolve the moved project by ID.
