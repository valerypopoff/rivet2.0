...
for the implemented work. The `idb` addition in `wrapper/web` is not an LOC
...
- `wrapper/api/src/web-app-auth-settings.ts`
- `wrapper/api/src/deployment-storage-settings.ts`
- `wrapper/api/src/public-route-settings.ts`
- `wrapper/api/src/runtime-limit-settings.ts`
- `wrapper/api/src/trusted-host-settings.ts`
- `wrapper/api/src/executor-url-override-settings.ts`
- `wrapper/api/src/node-executor-proxy-settings.ts`
- `wrapper/api/src/workflow-endpoint-auth-settings.ts`
...
`zod` is already a direct dependency of `wrapper/api`, but wrapper API code
...
`@tanstack/react-query` is already a direct dependency of `wrapper/web`
...
1. `wrapper/api/src/routes/workflows/managed/execution-cache.ts`
   - item-count LRU for endpoint pointers;
   - byte-budget LRU for revision materializations.
2. `wrapper/api/src/runtime-libraries/managed-code-runner.ts`
...
The package is currently a direct dependency of `wrapper/web`, not
`wrapper/api`. The API must add its own direct dependency; relying on a
...
- `wrapper/api/src/web-app-oauth.ts`: 686 LOC;
- `wrapper/api/src/server-ui-auth.ts`: 612 LOC;
- `wrapper/api/src/routes/ui-auth.ts`: 397 LOC.
...
Do not add it just to replace `wrapper/api/src/utils/exec.ts`. Windows command
...
- run `npm run test`;
- run `npm run verify:repo-structure`;
- run `git diff --check`.
...
- **TanStack Query:** `npm run verify:web-pure` plus Playwright coverage for the
